...
- [ ] `verdict`가 없으면 기존과 동일하게 동작한다(하위 호환, 기존 저장 리포트 불변)
- [ ] 서머리와 하이라이트가 서로 다른 1순위를 고르면 검사가 잡는다
- [ ] 정상 케이스·경계(선택지 1개)·오류(모순)·회귀(verdict 없는 기존 리포트) 4종 테스트
...
- [ ] 서머리가 리더 최상단과 PDF 인쇄본 모두에 나온다
- [ ] 서머리 생성 실패가 섹션 생성을 막지 않는다(부분 실패 격리)
- [ ] 기존 저장 리포트는 서머리 없이도 정상 렌더된다
- [ ] 서머리가 `verdict`와 같은 결론을 말한다(L1 검사 통과)
...
- [ ] 5개 서비스에만 하이라이트가 생기고 나머지 15개는 영향이 없다
- [ ] 하이라이트가 목차 항목을 요약만 하지 않는다(새 판단 포함)
- [ ] `shape`가 지정한 문단 수를 지킨다
- [ ] 하이라이트도 `verdict`와 결론이 같다
...
- [ ] 예산은 이 저장소에서 두 번 측정한 값이다(인계 값 그대로 복사 금지)
- [ ] 라벨 날것 노출이 0건이다
- [ ] 예산 미달·초과가 재생성으로 돌고, 재생성 한도 초과 시 `NOT_RUN`이 아니라 명시적 실패로 남는다
