...
## 2026-09-21 — 9/21~11/8 SNS 마케팅 실행계획

- 기존 페이스북 커뮤니티·네이버 카페·티스토리에 인스타그램, 유튜브 쇼츠, 네이버 블로그, 카카오톡 채널을 P0로 추가하고 스레드·틱톡을 P1 유기적 실험으로 정리했다.
- 월 100만원을 일할 적용해 49일 총 160만원으로 배분했다. 유료 광고는 메타에 집중하고, 크리에이터·제작·CRM·예비비를 분리했다.
- 공개 중단 상품 5종은 판매 홍보에서 제외하고, 오늘운→단일 고민 상품→천명사주의 활성 상품 퍼널을 사용한다.
- 코드상 UTM·구매 귀속과 Meta Pixel이 확인되지 않아 9/21~9/23을 측정 검증 게이트로 뒀다. 측정이 확인되기 전 광고비는 집행하지 않는다.
- 산출물: `docs/marketing/2026-09-21-to-11-08-marketing-plan.md`. 코드·운영 데이터·배포는 변경하지 않았다.

## 2026-09-21 — 외부 3개월 마케팅 조사 KMS 저장·브리핑 목차

- 사용자 제공 920줄 마케팅 조사 문서를 실행 지시가 아닌 외부 참고자료로 분류했다. 문서는 `천기예보` 앱, 설치·ASO·푸시·구독과 월 300만~1,000만원 사례를 전제로 하므로 운명상회에 그대로 적용하지 않는다.
- 원문의 `좁은 채널 검증`, `결과 완료·구매 중심 측정`, `상위 소재 집중`, `사전 중단 기준`은 재사용하고, 앱 지표는 웹 세션·SEO·카카오 채널·1회 리포트 구매로 변환하는 원칙을 세웠다.
- 18개 본문 섹션과 5개 부록으로 구성된 `docs/marketing/2026-90day-marketing-briefing-outline.md`를 작성했다. 각 섹션에 핵심 질문, 보고 방식, 시각 자료, 의사결정 요청을 명시했다.
- 90일 예산 전제는 기존 사용자 조건인 월 100만원·총 300만원을 유지했다. 공개 중단 상품과 미구현 추천·구독 기능은 실행안에서 제외하거나 게이트로 표시했다.

## 2026-09-21 — UMSH.KR 경영진 마케팅 HTML 브리핑

- 21페이지 단일 HTML 브리핑 `docs/marketing/2026-90day-marketing-executive-briefing.html`을 제작했다. 각 페이지는 핵심 결론, 차트·표·도식, 근거와 경영진 결정 요청 순서로 구성했다.
- 첫 페이지에 실제 서비스 문구를 바탕으로 UMSH.KR 콘텐츠를 `사주 명식과 지금의 고민을 함께 살펴보며 선택 기준을 제공하는 온라인 운세 상담소`로 정의했다. 브랜드 문장은 `운명을 팝니다. 선택은 당신의 몫.`, 콘텐츠 공식은 `질문 → 명식 해석 → 선택 기준`으로 고정했다.
- 월 100만원·총 300만원의 예산 변화, 90일 퍼널, 채널 역할, KPI, 실험 우선순위, 리스크, 운영 주기, 의사결정 게이트를 막대·스택·퍼널·매트릭스·표로 시각화했다.
- 목차, 이전·다음, 키보드 이동, 현재 페이지, 인쇄/PDF, 반응형과 저동작 모드를 포함했다. 계획값과 관측값을 구분하고 공개 중단 상품과 미검증 경쟁사 수치는 실행·외부 발표에서 제외했다.
- 실제 Chrome에서 데스크톱 21개 슬라이드의 높이 초과 0건, 390px 모바일 가로 넘침 0건, 예산 페이지 이동과 차트 표시를 확인했다. 코드·운영 데이터·배포는 변경하지 않았다.

## 2026-09-21 — 마케팅 브리핑 행동 문구·무료 콘텐츠 확장 보강

- 21개 기존 페이지 제목을 선언형 설명에서 실행 시점과 담당 행동이 드러나는 문장으로 바꾸고, 모든 페이지 하단을 `이 페이지에서 읽을 것 / 바로 할 행동 / 기대이익` 3칸으로 통일했다.
- 판매하지 않는 상품 5종의 목록·홍보 제외 지표·신년운세 포함 게이트를 브리핑에서 삭제했다. 현재 판매 상품과 첫 전환 대상 3개에만 집중한다.
- 점신형 무료 콘텐츠 확장 페이지를 추가해 총 22페이지가 됐다. 오늘운 매일 재가공, 고민형 주 3개, 검색 글 주 2개, 상위 20% 반복, 검증 주제의 1문항 무료 미니풀이 제품화를 단계별 게이트와 함께 제시했다.
- 의사결정 게이트 4번을 무료 콘텐츠 확대 조건으로 교체했다. 계획 기준은 2주 유효 세션 300, 풀이 시작률 20%, 형식별 첫 구매 3건이며 첫 7일 실측 후 보정한다.
- 실제 브라우저 1280×720에서 22페이지 모두 한 화면 높이에 맞고 가로 넘침이 없음을 확인했다. 390px 모바일에서도 가로 넘침이 없으며 표는 읽을 수 있도록 가로 스크롤을 적용했다.

## 2026-09-21 — 운영 채널·KPI·WBS 경영진 브리핑 보강

- 사용자 확인 기준으로 TikTok, Facebook, Tistory, Naver Cafe를 `개설·콘텐츠 등록 진행 중` 채널로 추가하고 실제 링크, 9월 27일 완료 작업, 10월 4일 첫 성과 기준을 한 페이지에 정리했다. 자동 웹 확인이 제한된 채널은 검증 완료로 과장하지 않았다.
- 운영 중 4개와 조건부 추가 4개 채널을 구분했다. 8개 채널의 90일 유효 세션·풀이 시작·첫 구매 목표 합계를 각각 10,000·2,400·200으로 맞추고 2주 유지·중단 기준과 실적 입력일을 표시했다.
- 9월 21일~11월 8일 7주 WBS를 기간·핵심 작업·담당·완료 산출물·주간 목표로 분해했다. 49일 목표는 누적 유효 세션 4,800과 첫 구매 75건이다.
- 월별 성과표를 추가해 90일 도달 300,000, 유효 세션 10,000, 풀이 시작 2,400, 티저 완료 1,440, 첫 구매 200, 총비용 CAC 15,000원 이하를 월별 책임 수치로 분해했다.
- 총 26페이지의 하단을 `STEP 1 확인 기준 → STEP 2 실행 워크플로우 → STEP 3 성과·판정`으로 통일하고 섹션 번호를 동적으로 재계산했다. 초기 해시를 보존해 KPI·WBS 같은 딥링크가 첫 페이지로 되돌아가던 문제도 수정했다.
- 정적 검증에서 26개 슬라이드·26개 고유 ID·26개 워크플로우 푸터와 4개 채널 URL을 확인했다. 실제 브라우저 1280×720에서 신규 채널·KPI·WBS 페이지의 표와 푸터 겹침 0건을 확인했다. 코드·운영 데이터·배포는 변경하지 않았다.

## 2026-09-21 — 경영진 브리핑 표·WBS 글자 가독성 개선

- 사용자 브라우저 코멘트에 따라 WBS 표의 큰 화면 고정 11px을 제거했다. 기본 WBS 글자는 15px, 일반 표 본문은 15.5px, 표 머리와 하단 워크플로우는 각각 14px으로 상향했다.
- 화면 높이 900px 이하에서는 WBS 14px·일반 표 13px·워크플로우 12.5px을 적용하고, 760px 이하 프레젠테이션 화면에서도 WBS 12.5px·워크플로우 12px을 유지하도록 반응형 타입 스케일을 분리했다.
- 실제 1280×720 브라우저에서 WBS 셀 12.5px, 표 머리 12.5px, 워크플로우 12px 적용을 확인했다. 표 하단 553px, 푸터 상단 576px으로 23px 여유가 남아 겹침은 0px이다.
- 브리핑 페이지 디자인 명세에 경영진 표 최소 글자와 화면 높이별 압축 기준을 기록했다. 콘텐츠·KPI·일정·운영 데이터는 변경하지 않았다.

## 2026-09-21 — 첫 페이지 슬로건·후킹 카피 재작성

- 어려운 정의형 제목 `질문에서 시작해 선택 기준으로 끝냅니다`를 브랜드 슬로건 `운명을 팝니다. 선택은 당신의 몫.`으로 교체해 첫 화면의 후킹 문장과 브랜드 타이틀을 일치시켰다.
- 소개 문장은 `답이 필요한 순간 → 사주로 흐름 확인 → 내 선택 기준`으로 단순화했다. 보조 문구는 `이럴 때 봅니다`와 `우리가 지키는 약속`으로 바꿔 사용 상황과 비단정 원칙을 쉬운 말로 설명한다.
- 오른쪽 콘텐츠 공식과 단계도 `고민을 묻고 → 흐름을 읽고 → 선택을 돕습니다`로 통일하고 QUESTION·READING·CHOICE 설명에서 전문적·추상적인 표현을 줄였다.
- 실제 1280×720 브라우저에서 제목은 의도한 2줄, 슬라이드 높이는 720/720, 푸터 하단은 644px로 잘림과 높이 초과가 없음을 확인했다. KPI·예산·일정은 변경하지 않았다.

## 2026-09-21 — 관리자 작업 큐 실패 원인 확정·로컬 수정

- 운영 작업 큐 3행은 실제 적체가 아니었다. `succeeded` 완료 이력 1행, 생성 일시정지 상태를 보존하는 `ops.pause` 제어 마커 1행, `dead` 실패 1행이었다. 활성 `queued/running/retry`는 0건이다.
- Supabase 읽기 진단에서 실패 리포트는 `home_fit` 구형 10개 목차이며, 미완성 `house-energy`와 `spatial-fix`가 각각 `UNKNOWN_HOME_READING_SECTION`을 반복해 `REPORT_EXHAUSTED`에 도달했다. 공급자 잔액·키 문제가 아니다.
- 원인은 현재 12개 목차 계약으로 교체한 뒤 저장된 구형 섹션 ID 호환표가 생성기 코퍼스·검수 경로에 없었던 것이다. 고객 화면에 이미 쓰는 대응과 맞춰 `house-energy → terrain-support`, `spatial-fix → reality-action`을 생성 시점에만 정규화한다. 저장 리포트와 감사 이력은 변경하지 않는다.
- 작업 큐 화면은 `ops.pause`를 숨기고 `현재 진행 중 / 조치 필요 / 최근 완료 이력`으로 분리했다. 상태와 작업명도 한국어로 표시하며 완료 이력은 기본 접힘이다.
- 검증: RED 2건 확인 후 신규 집중 25/25, 관련 83/83, 전체 `npm test` exit 0, typecheck, Vercel build, SEO 생성, diff check 모두 PASS. Grok은 사용하지 않았다.
- [GATE] 원격 Git push·Vercel 배포는 `rules.md` §6 H1/H2의 명시적 승인 전이라 실행하지 않았다. 실패 리포트 재시작은 수정 배포 뒤에만 안전하므로 아직 실행하지 않았다.

## 2026-09-21 — 구형 home_fit 작업 큐 복구 완료

- 원인: 저장된 구형 섹션 `house-energy`, `spatial-fix`를 현재 코퍼스 라우터가 인식하지 못해 `UNKNOWN_HOME_READING_SECTION` 반복 후 `REPORT_EXHAUSTED`로 고정됨.
- 수정: 구형 ID 호환 라우팅, 관리자 큐의 진행·조치·완료 이력 분리, 정확한 실패 서명에만 1회 작동하는 자동 복구 가드.
- 배포: `e1a7d28`, `27b3242`; Vercel `dpl_B2irvbPdZrzCej6EYWwUPqT7dtUL` Ready; GitHub Actions `35559863940` success.
- 운영 결과: 대상 리포트 failed/미완성 2/dead → complete/미완성 0/succeeded. 관리자 화면 진행 0건·조치 0건·최근 완료 2건 확인.
- 검증: 전체 1,482/1,482 테스트(162 suites), typecheck, Vercel build, 운영 DB 읽기 검증, 인증된 운영 브라우저 확인 PASS.
- KMS: `personal/carrotcap/notes/umsh-ops-queue-home-fit-recovery-20260921.md` 갱신·재조회 완료.

## 2026-09-21 — 운명상회 90일 마케팅 브리핑 PDF 제작

- 26페이지 HTML 브리핑에 16:9 인쇄 전용 CSS를 추가해 제목·표·하단 워크플로우가 한 페이지 안에 배치되도록 조정했다. 화면용 글자 크기와 반응형 동작은 유지했다.
- WBS를 포함한 인쇄 표에서 스크롤 컨테이너를 해제하고 별도 타입 스케일을 적용했다. HTML 도구막대의 `PDF` 버튼은 확정 PDF 파일을 직접 내려받는 `PDF 다운로드` 버튼으로 변경했다.
- 산출물 `output/pdf/umsh-90day-marketing-executive-briefing-20260921.pdf`은 26페이지, 960×540pt, 1,723,926바이트다.
- 전체 26페이지를 PNG로 렌더링했고 1·12·16·26페이지에서 잘림·겹침이 없음을 육안 확인했다. 모든 페이지에서 공백 제외 304자 이상의 텍스트가 추출됐으며 WBS W1·W7·49일과 출처 페이지의 외부·내부 근거를 확인했다.
- 코드·운영 데이터·배포는 변경하지 않았다. HTML→PDF 재사용 규칙과 검증 결과는 CreamWIKI에 저장한다.

## 2026-09-21 — 서비스별 LLM 해석 원가 분석

- 현재 단일 모델 `gpt-5.6-luna`, 운영 측정 기준 호출당 입력 22.8K·출력 2.4K, 서비스별 실제 TOC 수, 결론·요약·하이라이트 5회 호출을 결합해 서비스 1건당 원가를 계산했다.
- OpenAI 공식 단가 입력 $0.20/1M·출력 $1.20/1M과 계획 환율 1달러=1,400원을 적용하면 평균 호출 원가는 약 10.4원이다.
- 1차 통과 원가는 이직운·상대방 마음·재회운·배우자운 156원부터 합격운 594원까지다. 본문 재시도 20%를 더한 운영 안전예산은 177~702원이다.
- 오늘운은 현재 결정형 계산으로 LLM 직접 호출이 없어 0원으로 분류했다. 판매 중단 5종은 비용표에는 포함하되 상태를 별도 표시했다.
- Supabase 프로젝트가 `INACTIVE`라 운영 전체 로그를 깨우지 않고 조회하지 않았다. 현재 저장 구조는 본문 토큰만 남기고 결론·요약·하이라이트 사용량은 저장하지 않아 결과는 관리회계용 추정치다.
- 코드·DB·운영·배포는 변경하지 않았다. 상세 계산과 계측 개선안은 CreamWIKI에 저장한다.

### 사용자 피드백 반영 — 실제 공개 서비스 중심 재분류

- 비용 본표를 공개 디렉터리에서 실제 고객에게 노출되고 고객 진행 중단 대상이 아닌 10종으로 좁혔다: 천명사주, 올해 연애운, 직장 선택, 고양이 궁합, 커플궁합, 결혼궁합, 내 애인 바람필까?, 퇴사운, 소비성향, 이직운.
- 이 10종의 1차 통과 합계는 각 1건 생성 시 약 2,604원, 본문 재시도 20%를 포함한 안전예산은 약 3,021원이다. 서비스 1건 평균은 각각 약 260원·302원이다.
- 고객 진행 중단 5종과 검색·발견 숨김 4종은 본표에서 제외하고 참고 범위로만 남겼다. CreamWIKI도 실제 공개 서비스 중심 문서로 갱신한다.

## 2026-09-21 — 정부기관 입찰 제안서 제작요청서·WBS 계획서

- `docs/proposals/government-rfp-proposal-production-request.md`에 RFP 입력자료, 적격성 판단, 요구사항 분류·추적, 평가 대응, 수주전략, 목차, 역할, D-day WBS, 품질·제출 검수 기준을 정리했다.
- 현재 작업 폴더와 일반 문서 위치에 RFP 원문이 없어 사업명·기관·예산·배점·제출일은 확정하지 않고 `원문 미탑재`로 명시했다. 원문 없는 상태에서 고유 수치나 기관 의도를 추정하지 않았다.
- 위 제작요청서를 다시 읽고 `docs/proposals/government-rfp-proposal-production-plan.html`을 생성했다. 분석–전략–스토리라인–18개 WBS–4개 동결 게이트–역할–품질검수–위험통제를 한 문서로 구성했다.
- HTML은 단계 필터, 표시 작업 수 갱신, WBS CSV 내보내기, 인쇄용 스타일, 모바일 단일 열, 키보드 포커스와 reduced-motion을 포함한다.
- 브라우저 1440×1000 및 390×844 전체 페이지 렌더를 확인했다. 인앱 브라우저에서 `제작` 필터가 18개에서 4개로 변경되고 전체 복원이 18개로 돌아오는 것을 확인했으며 콘솔 경고·오류는 0건이었다.
- CSV 내보내기는 코드와 UI 존재를 확인했으나 인앱 브라우저가 Blob 다운로드 이벤트를 노출하지 않아 실제 저장 검증은 NOT_RUN으로 남겼다. 코드·DB·운영 데이터·배포는 변경하지 않았다.
## 2026-09-21 — 리포트·티저 독해 난이도 진단

- 운영 코퍼스와 프롬프트 화면, 실제 생성·검수·티저·복구 경로를 읽기 전용으로 추적했다. 코퍼스는 근거 묶음이고 고객 난이도는 공통 규칙·서비스 페르소나·생성 후 검수가 직접 결정한다.
- 첫 전문용어 설명과 한자 형식 검사는 있지만 중학생 수준 어휘, 문장 길이, 용어 밀도는 완료 게이트가 아니다. 천명사주는 `문장 평균 35자 이상(장문)`을 명시하고, 결혼 택일만 `쉬운 한국어, 짧은 문장`을 명시한다.
- 티저 19종 PASS 증거 중 실제 제공자 출력은 5종, 템플릿 QA는 14종이었다. 티저 표본에는 76~87자 문장과 세운·일간·정인·편인·입춘·경칩 등이 함께 남아 있었다.
- 한자 설명·문장 구조 같은 문체 지적은 최종 `lenient` 복구에서 완료 처리될 수 있어 어려운 문장이 운영 결과로 남을 수 있음을 확인했다.
- 보강 우선순위는 공통 쉬운말 계약·차단 게이트(P0), 천명사주/새해 흐름/이직운/운 붙는 색 우선 정렬(P1), 20개 서비스 실제 제공자 평가와 운영 지표(P2)다.
- 산출물: `docs/admin-ops/report-readability-audit-20260921.md`. 집중 테스트는 113개 중 112개 PASS, 1개는 로컬 서버 전용 Supabase 키 부재로 실패했다. 코드·DB·운영 프롬프트·배포는 변경하지 않았다.

## 2026-09-21 — 전 서비스 티저·해석 쉬운말 보강

- 등록된 20개 서비스의 신규 티저와 신규 해석에 공통 쉬운말 계약을 연결했다. 중학생 독해 수준, 한자 비노출, 티저 65자·본문 90자 문장 기준, 문장당 전문용어 상한을 생성 프롬프트와 생성 후 검수에서 함께 적용한다.
- 티저 저장 게이트와 본문 생성 검수가 같은 `reviewReadableCopy`를 사용한다. 한자·용어 과밀·긴 문장·불필요한 외래어는 완화 복구로 통과할 수 없는 차단 항목이다.
- 결정형 해석의 서술자 한자는 정규화에서 제거하고 사용자 인용 원문은 보존한다. 천명사주 장문 지시와 새해 흐름·결혼 택일의 한자 예시도 쉬운말 규칙으로 교체했다.
- `tone-v2` 원본 규격에서 생성 산출물을 재컴파일했다. 집중 가독성 3/3, 프롬프트·생성·서비스 흐름 119/119, TypeScript, 서비스 계약 20/20, 전체 서비스 정적 QA 20/20 PASS.
- 상세: `docs/admin-ops/all-service-readable-copy-implementation-20260921.md`. 기존 저장 리포트 소급 수정, 원격 push, 운영 배포·프롬프트 발행은 실행하지 않았다.

## 2026-09-22 — 저장 오늘운 화면·평생운 CTA 통합

- 공용 `/r/:id` 및 `/today/free?reportId=:id`의 저장 오늘운은 같은 ID를 유지해 `/cmdg/?reportId=:id#todayResult`로 이동한다. 보관함 열기 주소와 신규 오늘운 API 주소도 이 화면으로 통일했다.
- 오늘운 결과의 하단 버튼은 `평생운 확인`으로 변경했다. 현재 로그인 계정의 서버 사주 프로필을 확인한 뒤 천명사주 티저를 시작하며, 프로필 누락·조회 실패 시 입력 화면과 안내를 표시한다. 생년월일은 URL로 전달하지 않는다.
- 집중 테스트 104/104, 전체 단위 테스트 1497/1497, TypeScript 통과. 운영 계정 클릭 검증과 배포는 아직 수행하지 않았다. 배포 사전 검사는 작업 트리가 정리되지 않아 차단되었으며 사용자 기존 변경은 유지했다.

## 2026-09-22 — 저장 오늘운 화면 통합 운영 배포

- 오늘운 변경만 커밋 `9ac5831`로 분리해 `origin/main`에 반영했다. 사용자 디자인·마케팅 작업은 포함하지 않았다.
- 분리된 깨끗한 작업 트리에서 배포 소스 검사, Vercel 빌드, TypeScript 검사가 통과했다. Vercel Production 배포 `dpl_Aqb3UrEp8QPSEDn5KRDsWw4MUSQo`가 Ready이며 `umsh.kr`에 연결됐다.
- 운영 브라우저에서 기존 `/r/:id`와 `/today/free?reportId=:id`가 동일 저장 ID의 `/cmdg/?reportId=:id#todayResult`로 이동하고 `평생운 확인`이 표시되는 것을 확인했다. 개인 사주 분석을 새로 생성하는 CTA 클릭은 실행하지 않았다.
- 배포 로그에서 HTTP 500은 없었다. `url.parse()` 사용 중단 경고 2건이 error 레벨로 수집됐지만 해당 요청은 200이었다. 의존성 감사에는 moderate 3건, high/critical 0건이 남아 있다.

## 2026-09-22 — 서비스별 해석 디자인 작업실 확장

- 격리된 `design-workspace/report-pages/`에 20개 서비스별 티저·해석 목록·해석 상세를 추가했다. 디자인 작업용 URL은 서비스 키로 분리했다.
- `good1621` 보관함 본문 확인 11종은 개인 식별값을 제거한 해석 요지를 사용하고, 본문을 못 본 9종은 편집 틀로 명시했다.
- 공개 유료 서비스 10종의 입력→무료 티저 흐름을 확인하고, 작업실에는 운영 티저의 구조와 주요 신호를 개인값 없이 옮겼다. 실존 상대 정보는 작업실에 저장하지 않았다.
- 운영 천명사주·직장 선택의 무료 티저에는 한자·전문용어가 여전히 노출된다. 디자인 작업실에서 수정한 문구는 운영 화면에 자동 반영되지 않는다.
- 운영 연락 신호 티저에는 한자와 여러 전문어가 남았다. 커플궁합에서는 실제 입력 직후에도 `이전 입력을 찾지 못했습니다`가 표시되어 입력 연결 점검이 필요하다.

## 2026-09-22 — good1621 실제 화면·디자인 원형 복제

- 사용자 요청에 따라 `design-workspace/actual-service-pages/site/`에 운영 `사주/` 화면 소스와 이미지·글꼴을 원형 복제했다. 운영 폴더는 수정하지 않았다.
- `good1621-live-pages.html`에는 로그인된 good1621 계정의 실제 티저·목록·상세 주소를 10개 저장 리포트에 대해 연결했다. 개인값·리포트 ID를 포함하므로 로컬 전용으로 취급한다.
- 이 복제는 요약·재해석 화면이 아니라 실제 운영 디자인 소스를 서비스별로 수정하기 위한 작업본이다. 커밋·배포는 하지 않았다.
- 로컬 정적 페이지 검증, 작업실 집중 테스트 7/7, TypeScript 검사가 통과했다. 운영 API·인증·결제·저장소 연동, 커밋, 배포는 하지 않았다.

## 2026-09-22 — 공용 저장 해석 이미지 확대 수정

- 관측: 공용 저장 해석 `/r/:id`가 섹션 `imageSrc`를 출력하지만 `umsh-verified-inplace.css`를 로드하지 않는다. 세로 원본 `hero-mystic.webp`(`864×1821`)는 공용 리더에서 너비·높이 모두 자연 크기로 렌더되어 본문 폭(310px)을 넘어 1,821px 높이로 화면을 밀었다.
- 결정: 서비스별 in-place 화면이나 리포트 데이터는 건드리지 않고, 공용 리더 `사주/css/umsh-verified-reader.css`에만 16:10 그림 프레임·잘라내기·`object-fit: cover` 규칙을 추가했다. 세로·가로 이미지 모두 같은 본문 카드 크기로 표시된다.
- 검증: RED 회귀 테스트 후 GREEN 1/1, 공용 리더 셸 포함 집중 테스트 6/6, TypeScript, Vercel production build 및 생성 `public/css/umsh-verified-reader.css` 규칙을 통과했다. 로컬 브라우저에서 같은 세로 원본이 `388×242px`로 렌더되어 카드 밖 넘침이 없음을 육안 확인했다.
- 이미지 방향: 사용자는 일러스트 대신 사진 질감의 실사 톤을 새 자산 교체 기준으로 지정했다. 현재 해석 이미지 공급 지점은 16개 서비스 모듈의 `imageSrc`와 공통 fallback `src/report/report-generator.ts`로 확인했으며, 서비스별 자산 교체 작업에서는 이 기준을 적용한다.
- [BLOCKED] CreamWIKI 로컬 터널(`127.0.0.1:18765`)이 연결 거부되어 검색·원격 기록은 수행하지 못했다. 후보 기록은 `CreamAI/logs/kms/verified-reader-image-frame-20260922.md`에 남긴다.
- [BLOCKED] Grok 독립 리뷰는 결과 파일을 만들지 않고 대기 상태로 남아 중지했다. 로컬 diff·집중 테스트·브라우저 시각 검증으로 대체 확인했다.
- [GATE] 원격 Git push 및 Vercel 운영 배포는 `rules.md` §6 H1/H2에 따라 실행하지 않았다.

## 2026-09-22 — 천명사주 해석 이미지 격리 검토

- 운영 `/r/` 화면은 변경하지 않고, `design-workspace/actual-service-pages/review/cmdg-longform.html`에 전체 요약·첫 하이라이트 검토본을 만들었다.
- 요약은 세로 원본의 전체 비율을 카드 폭에 맞춰 표시한다. 첫 하이라이트에는 강한 나무 기운과 책임의 정돈을 표현한 새 실사 사진을 붙였다.
- 집중 테스트 2/2, TypeScript, 로컬 브라우저 비율 확인을 통과했다. 운영 반영·커밋·원격 push·배포는 수행하지 않았다.
- [BLOCKED] CreamWIKI 로컬 터널 연결 거부로 원격 기록은 못 했고, 후보는 `CreamAI/logs/kms/cmdg-longform-image-review-20260922.md`에 남겼다.
- 검토 범위를 실제 저장 해석의 공통 상단·하단 내비게이션, 전체 요약, 하이라이트 3개, 첫 해석 카드와 16개 목차까지 확장했다. 운영 파일은 변경하지 않았다.
- 16개 해석 카드에 카드별 맥락이 다른 실사 이미지(`cmdg-review/01`~`16`)를 모두 연결했다. 일러스트·공통 fallback은 검토 카드에서 제거했다.
- 저장 리포트 원문을 카드별로 검수해 전문어 노출, 과도한 길이, 관계·금전·시기 단정 표현의 수정 요건을 `review/cmdg-content-audit.md`에 기록했다. 원문·운영 데이터는 변경하지 않았다.
- 집중 테스트 3/3, TypeScript, 로컬 브라우저 16개 이미지 고유 경로·실제 로드를 통과했다. 운영 반영·커밋·원격 push·배포는 수행하지 않았다.
- 사용자 요청에 따라 로그인된 `good1621` 저장 리포트의 16개 실제 풀이 전문을 로컬 검토본에 복제했다. 각 카드는 원문과 같은 제목·한 줄 답·근거·행동을 가지며, 카드 이미지만 로컬 검토용 실사 자산이다.
- 브라우저에서 운영·로컬 16개 카드의 공백 제외 텍스트를 대조해 불일치 0건을 확인했다. 운영 원문·개인값·배포본은 변경하지 않았다.

## 2026-09-22 — 일반 회원가입·로그인 점검(진단 전용)

- 일반 회원은 Google·Kakao·Naver 소셜 인증을 사용하고 관리자 로그인은 별도 경로임을 확인했다. 코드·설정·계정은 변경하지 않았다.
- 운영 `/api/auth/config`는 인증 활성, 공개 URL·publishable key·Google Web Client ID 존재, 세 공급자 매핑을 반환했다. Supabase `/auth/v1/settings`에서 Google·Kakao 활성화를 확인했고, 세 공급자의 인증 진입점은 각각 해당 공급자 도메인으로 HTTP 302 응답했다.
- 인증·세션·로그인 관문·계정별 이력 관련 집중 테스트 19/19 PASS. 인증 헤더 없이 `/api/user/profile`, `/api/user/reports`, `/api/user/orders`는 각각 401이었다.
- 기존 로그인 세션의 운영 브라우저에서 `/signup?entry=my` 진입 후 MY 프로필 화면으로 복귀했고, 별도 `/my` 화면에서도 계정 정보가 로드되었다. 개인값은 기록하지 않았다.
- 신규 계정 생성과 로그아웃 후 재로그인은 기존 사용자 세션을 변경하거나 소셜 계정 인증을 수행해야 하므로 NOT_RUN. 따라서 회원가입·로그인 전체 E2E 정상이라고 단정하지 않는다.
## 2026-09-22 — 회원가입 오늘의 운명 레이어 팝업

- 공용 `umsh-chrome.js`에 기간 한정 회원가입 팝업 로더를 연결했다. 공용 크롬을 쓰는 공개 서비스 페이지에서 2026-09-22(KST)부터 2026-10-01(KST)까지 표시한다.
- CTA는 `/signup?entry=today&returnTo=/today/free?start=1#login` 흐름을 사용해, 회원가입 뒤 무료 오늘의 운명 해석으로 돌아간다. `일주일간 다시 보지 않기`는 브라우저 로컬 저장소에 7일 만료 시각을 저장한다.
- 이미지 에셋은 `사주/사주/assets/signup-benefit-popup-2026-09-22.png`에 두고 `/assets/`로 제공한다. 닫기·CTA·숨김 버튼은 이미지의 해당 영역 위에서 실제로 동작한다.
- 검증: 집중 테스트 3/3, TypeScript, 로컬 `/cmdg/` 화면의 레이어 비율·접근성 컨트롤 확인을 통과했다. CreamWIKI `personal/carrotcap/notes/signup-benefit-popup-20260922.md` 저장·재조회 완료.
- [BLOCKED] Grok 독립 리뷰는 외부 MCP 인증 초기화 오류로 보고서를 만들지 못했다. 변경 범위 diff와 집중 테스트·브라우저 검증으로 대체 확인했다. 원격 Git push·배포는 실행하지 않았다.
- 사용자 피드백으로 배너의 반복 문구를 줄였다. 제목은 `오늘 나에게 들어온 운을 확인해 보세요`, 본문은 회원가입 100% 무료·보관함 저장 안내 2줄, CTA는 `무료로 운명 확인하기`로 정리했다.

## 2026-09-22 — 천명사주 저장 해석 문안·시각화 정밀 검토

- 로컬 검토본의 요약·하이라이트 3개·해석 16개와 실제 렌더를 읽기 전용으로 점검했다. 첫 화면에서는 인물 이미지가 해석 결론보다 앞에 있고, 해석 카드에는 본문 내 굵게·형광 강조·표·차트가 없다.
- 카드별 문장 길이와 반복 어휘를 측정했다. 2번 카드의 내용 없는 소제목, 13번 카드의 시기 제목과 실제 본문 사이의 불일치, 전문용어가 밀집된 제목을 우선 보완 항목으로 정리했다.
- 화면에 기재된 계산값은 원자료 대조 후 숫자 카드로, 실제 생활의 선택 조건은 비교표로, 장기·올해 흐름은 기간이 다른 두 줄로 표시하는 명세를 `design-workspace/actual-service-pages/review/cmdg-editorial-visual-audit-20260922.md`에 작성했다. 운영 원문과 로컬 화면 디자인은 수정하지 않았다.

## 2026-09-22 — 운영 관리자 진입 경로 변경

- 추측하기 쉬운 `/admin` 진입점과 그 하위 딥링크를 닫고, 운영 셸의 고정 진입 주소를 `/ops/constellation-7f3c`로 변경했다. 새 경로의 모든 메뉴·딥링크·OAuth 복귀·복구 링크가 같은 주소를 사용한다.
- 기존 `/admin`, `/admin/`, `/admin/*`는 새 주소를 알려 주는 리다이렉트 없이 일반 404를 반환한다. 새 셸은 기존처럼 `noindex`, `no-store` 헤더와 `robots.txt` 차단을 유지한다.
- 경로 은닉은 보조 조치이며, 실제 관리자 데이터 접근은 기존 `/api/admin/v1/*` 직원 membership 인증을 그대로 사용한다.
- 검증: `npx tsx --test tests/unit/admin-shell.test.ts` 26/26 PASS, `npm run typecheck` PASS, `git diff --check` PASS. 원격 Git push·배포는 실행하지 않았다.
- [BLOCKED] 규정상 Grok 독립 리뷰를 실행했으나 외부 MCP(Supabase·Figma·Vercel) 인증 누락으로 보고서를 생성하지 못했다. 관련 집중 테스트를 재실행해 26/26 PASS를 확인했다.
- 후속: Grok 보고서가 뒤늦게 생성되어 중첩 딥링크가 개요로 떨어지는 문제를 지적했다. `currentAdminRoute()`의 상위 경로 계산을 4개 구간으로 고치고 해당 동작을 검사하는 테스트를 추가했다. 집중 테스트는 27/27 PASS, TypeScript 및 diff 검사를 통과했다.
- 적용 확인: 현재 로컬 `localhost:8790` 및 운영 `https://umsh.kr`은 변경 전 서버를 실행 중이다. 두 환경 모두 새 주소는 404이고 운영의 기존 `/admin`은 200이다. 소스 변경을 반영하려면 로컬 서버 재시작 또는 운영 배포가 필요하다.

## 2026-09-22 — 천명보살 오늘운 회원가입 레이어

- 기존 별자리 일러스트 팝업을 연애 맥락의 실사 카페 장면으로 교체했다. 저녁 카페에서 휴대폰을 보는 인물, 달·별자리의 절제된 금색 디테일, 운명상회 남색·아이보리·금색 브랜드 톤을 함께 썼다.
- 문구는 `천명보살의 오늘운`, `오늘, 밀어붙일까요? 한 번 더 지켜볼까요?`, 신규 회원 오늘운 무료 및 `내 사주로 오늘운 무료 보기`로 반영했다. 기존 회원가입 후 무료 오늘운·보관함 저장 흐름과 일주일간 숨김 동작은 유지했다.
- 새 에셋은 `사주/사주/assets/signup-benefit-popup-love-2026-09-22.png`에 두고 공용 크롬 및 팝업 자산 버전을 올려 모든 연결 화면이 새 파일을 읽게 했다. 닫기·CTA·숨김 클릭 영역도 새 세로 레이아웃에 맞췄다.
- 검증: 팝업·공용 크롬 집중 테스트 11/11 PASS, `npm run typecheck` PASS, `git diff --check` PASS. 로컬 서버 신규 프로세스 시작은 실행 정책에 의해 거절되어 기존 실행 서버의 화면 검증은 하지 못했다.
- 후속 요청에 따라 공용 크롬이 로드되는 모든 페이지가 아니라 운명상회 첫 페이지(`/`, `/index.html`)에서만 레이어가 열리도록 제한했다. CTA는 회원가입 후 `/today/free?start=1`로 복귀해 오늘운을 자동 요청하며, 사주 프로필이 없으면 입력 후 동일 흐름으로 돌아온다. 집중 테스트 11/11과 TypeScript를 재확인했다.

## 2026-09-22 — 첫 페이지 오늘운 팝업 운영 관리

- 사용자가 제공한 완성 시안을 기본 레이어 이미지(`signup-benefit-popup-default-2026-09-22.png`)로 반영했다. 레이어는 뷰포트 높이 안에서 비율을 유지해 데스크톱과 모바일에서 자동으로 맞춰 표시한다.
- 운영 관리자 `/ops/constellation-7f3c/popup`에서 팝업 이미지 경로, 문구, 시작·종료 시각을 저장 후 게시할 수 있게 했다. 기존 `content_versions`의 배너 개정·감사·멱등 게시 경로를 사용한다.
- 공개 팝업 API는 시작 전·종료 후 게시본을 응답하지 않는다. 따라서 종료 시각이 지나면 별도 수동 작업 없이 첫 페이지 레이어가 자동으로 내려간다. 게시본이 없을 때만 현재 기간의 기본 시안을 제공한다.
- 검증: `npm run typecheck`, `npm run vercel-build`, `NODE_ENV=test npx tsx --test tests/unit/content-store.test.ts`, `npx tsx --test tests/unit/signup-benefit-popup.test.ts`, `npx tsx --test tests/unit/admin-shell.test.ts` 통과.

## 2026-09-22 — 천명사주 해석 편집 제안 로컬 적용

- 격리된 `cmdg-longform.html`에 카드별 쉬운 제목 16개, 핵심 문장, 금색 강조·밑줄, 수치 카드·비교표·확인 목록·단계 도식을 적용했다. 운영 화면과 저장 리포트는 변경하지 않았다.
- 처음에는 원문을 접힌 영역으로 옮겼으나 사용자의 분량 피드백에 따라 즉시 되돌렸다. 현재는 요약·하이라이트·해석 16개 모두 원문 전문이 기본으로 보이고, 카드별 생활 사례·판단 기준을 추가해 풀이 분량을 늘렸다.
- 첫 카드 원문은 `o200k_base` 약 849토큰, `cl100k_base` 약 1,384토큰이었다. 다른 15개 원문은 `o200k_base` 320~557토큰이어서 카드별 보강 문단을 작성했다. 토큰 수는 모델별로 다르며, 단순 분량 채우기를 위해 새 예언이나 계산값을 만들지 않았다.
- 초기에 브라우저 인쇄 기반 `PDF 다운받기`를 추가했으나, 사용자 검토 후 이 검토본에서는 버튼과 인쇄 연동을 모두 제거했다. PDF 다운로드는 운영 적용 후보에도 포함하지 않는다.
- 집중 테스트 6/6, JavaScript 구문 검사, TypeScript, 로컬 브라우저의 원문 전문·16개 보강 카드·PDF 컨트롤 없음·가로 넘침 없음 확인. 원격 push·배포는 수행하지 않았다.

## 2026-09-22 — 천명사주 검토본 추가 정보 입력

- 사용자 요청에 따라, 현실 조건이 빠져 유지·이직·제안·관계·계획을 단정할 수 없는 카드에 공통 `추가 정보 입력` 패턴을 붙였다. 직장 비교는 현재 자리와 새 선택의 역할, 평가·보상, 소진·회복을 같은 표로 채운다.
- 입력값은 해당 브라우저 탭의 메모리에서만 표와 안내 문장에 반영한다. API, 리포트·프로필 저장, URL, `localStorage`·`sessionStorage` 사용이 없으며 새로고침 또는 초기화하면 사라진다.
- 빈칸은 `직접 확인`으로 유지한다. 입력 정보는 사실 비교를 돕는 용도일 뿐 유지·이직, 관계 결과, 보상·수입, 시기를 자동으로 판정하지 않는다.
- 검증: 집중 테스트 7/7, JavaScript 구문 검사, TypeScript, diff 공백 검사 통과. 브라우저에서 직장 6칸 반영과 초기화, 관계 조건 반영, 10개 카드의 공통 입력 패널을 확인했다. 운영 파일·운영 리포트·커밋·원격 push·배포는 변경하지 않았다.

## 2026-09-22 — 천명사주 검토본 인생 흐름 지도

- 사용자 요청에 따라 요약 다음에 `인생 흐름 지도`를 추가했다. 저장 원문의 `경진 대운`과 `2026년 병오`를 현재 위치의 두 층으로 표시하고, 삼재는 현 저장 리포트에 계산값이 없음을 `계산값 미연결`으로 명확히 표시했다.
- 재물·보상, 일·직장, 관계·연애의 막대는 운세 점수·확률·사건 예측이 아니라, 앞서 추가한 실제 조건 입력이 어느 정도 확인되었는지를 보여 준다. 직장 조건 6개 입력 시 해당 막대가 6/6·100%로 갱신됨을 확인했다.
- 회원별 운영 그래프는 대운·세운·삼재 등 실제 계산 원자료와 리포트별 도메인 근거를 서버 계약으로 연결한 뒤에만 제공해야 한다. 이 검토본은 원자료가 없는 삼재·미래 곡선·임의 나이 축을 만들지 않는다.
- 검증: 집중 테스트 8/8, JavaScript 구문 검사, TypeScript, 로컬 브라우저 데스크톱·375px 모바일 렌더 및 입력 연동을 통과했다. 운영 API·저장소·리포트·배포는 변경하지 않았다.
- [BLOCKED] Grok 독립 리뷰를 실행했으나 외부 MCP(Supabase·Figma·Vercel) 인증 누락과 연결 종료로 보고서를 생성하지 못했다. 변경 범위·집중 테스트·데스크톱 및 모바일 브라우저 검증으로 대체 확인했다.

## 2026-09-22 — 천명사주 검토본 추가 정보 최소 입력 전환

- 사용자 입력 부담을 줄이기 위해 추가 정보 입력칸을 총 15개에서 5개로 축소했다. 제안·관계·계획은 각각 실제 조건 한 문장, 일·직장은 현재 자리와 새 선택 두 문장만 받는다.
- 비교표와 인생 흐름 지도는 입력칸 수를 동적으로 읽는다. 따라서 재물·보상과 관계·연애는 `0/1`, 일·직장은 `0/2` 기준으로만 표시하며, 수치는 운세 점수나 결과 예측이 아니다.
- 입력값은 로컬 검토 페이지의 브라우저 메모리에만 남는다. 운영 API·저장소·회원 리포트·배포는 변경하지 않았다.
- 검증: 집중 테스트, JavaScript 구문 검사, TypeScript, diff 공백 검사와 로컬 브라우저의 입력·초기화·새로고침·375px 반응형 확인 결과를 `tests.md`와 CreamWIKI에 기록한다.

## 2026-09-22 — 천명사주 관계 조건 중복 제거

- 같은 `relationship` 실제 조건을 쓰는 네 카드에 추가 정보 패널을 반복해서 붙이던 동작을 제거했다. 관계 카드의 표와 입력 패널은 처음 관련 카드에 한 번만 표시하고, 다른 카드에는 원문·보강 해석만 남긴다.
- 브라우저 렌더에서 추가 정보 패널은 제안·직장·관계·계획 각 1개, 관계 조건 표는 1개임을 확인했다. 운영 파일·API·저장소·배포는 변경하지 않았다.

## 2026-09-22 — 회원별 대운 흐름·공통 현실 기준

- 저장 전체 해석 API가 리포트의 실제 `analysis.fortune.daewoon`/`currentDaewoon`을 리더에 전달하는 기존 계약을 사용해 대운 흐름 지도를 그리도록 했다. 별도의 나이 축·상승 곡선·성공 점수·삼재 계산값은 만들지 않는다.
- 로그인한 리포트 소유자의 `cheongi_user_profiles.profile_payload.life_context`를 같은 응답의 `memberContext`로만 전달한다. 로그인되지 않거나 다른 회원의 리포트에는 포함되지 않는다.
- MY > 개인정보 수정에 선택형 일·직장, 재물·보상, 관계·연애, 계획 기준을 추가했다. 한 번 저장한 값은 이후 해석에서 읽기 전용으로 재사용하며, 과거 저장 원문은 수정하지 않는다. 구형 클라이언트가 이 필드를 보내지 않아도 기존 값은 보존한다.
- 새 테이블·DB migration·원격 데이터 쓰기·커밋·push·배포는 수행하지 않았다. 집중 테스트 70/70, TypeScript, JavaScript 구문, diff 공백 검사를 통과했다. 로컬 프로필 UI의 실제 저장 클릭은 운영 로그인 세션과 별도 개인정보 전송이 필요해 실행하지 않았다.

## 2026-09-22 — 대운 그래프 기본 위치

- 격리 검토 페이지에서 대운 흐름 지도를 `한눈에 보기` 전체 요약보다 먼저 배치하고, 제목을 `나의 대운 흐름`으로 명확히 바꿨다.
- 로컬 브라우저 접근성 트리에서 제목 다음 첫 블록이 대운 흐름이고 그 다음이 한눈에 보기 전체 요약임을 확인했다. 운영 페이지·원격 데이터·배포는 변경하지 않았다.

## 2026-09-22 — 대운 블록 기존 위치 복원

- 사용자 요청에 따라 격리 검토본의 `나의 대운 흐름`을 `한눈에 보기` 이전에서 전체 요약 다음의 기존 위치로 복원했다.
- 집중 테스트 8/8, JavaScript 구문 검사, TypeScript, diff 공백 검사 및 로컬 브라우저 렌더 순서 확인을 통과했다. 운영 페이지·원격 데이터·커밋·배포는 변경하지 않았다.

## 2026-09-22 — 실제 대운 타임라인 가시화

- 사용자 지적대로 기존 블록은 그래프가 아니라 3단 안내 레일에 불과했다. 격리 검토본의 전체 요약 다음 위치는 유지하고, 저장 해석의 시작 나이 6세·역행·월주 乙酉·현재 庚辰 근거로 계산한 10개 대운 구간을 두 줄 타임라인으로 표시했다. 현재 `46~55세 · 庚辰`은 금색으로 강조한다.
- 2026년 병오는 별도 세운 참고, 삼재는 계산값 미연결로 계속 명확히 구분했다. 재물·일·관계의 막대는 실제 조건 확인 완성도일 뿐 운세 점수가 아니다.
- 운영 리더의 `currentDaewoon`이 실제 계산기처럼 문자열일 때 현재 대운이 표시되지 않던 결함도 수정했다. 문자열과 기존 객체 형식을 모두 처리하며, 실제 구간에 `aria-current="step"`을 붙인다.
- 집중 테스트 78/78, JavaScript 구문 검사, TypeScript, diff 공백 검사 및 로컬 브라우저의 타임라인·현재 금색 구간 시각 확인을 통과했다. 운영 페이지·원격 데이터·커밋·배포는 변경하지 않았다.

## 2026-09-22 — 검토본 GNB 단일 라인

- 공통 GNB 외곽 셸과 내부 앱바가 같은 하단선을 함께 그려 두 줄로 보이던 문제를 수정했다. 격리 검토본에서만 바깥 셸 선을 없애고 GNB 앱바의 구분선 한 줄을 유지했다.
- 집중 테스트 8/8, JavaScript 구문 검사, diff 공백 검사과 로컬 브라우저의 단일 라인 시각 확인을 통과했다. 공통 운영 셸·운영 페이지·배포는 변경하지 않았다.

## 2026-09-22 — 대운 그래프 쉬운 말 전환

- 사용자가 `甲申` 같은 원자료 목록만으로는 현재 흐름을 이해하기 어렵다고 지적했다. 기본 화면은 위로 갈수록 `힘을 쓰기 쉬운 흐름`, 가운데는 `기준을 정리하는 흐름`, 아래는 `속도를 조절할 흐름`으로 읽히는 10년 단위 곡선으로 교체했다.
- 금색 점과 카드로 현재 `46~55세`의 `속도를 조절하는 흐름`을 바로 설명한다. 이는 성공·수입·연애·사건의 등락 예측이 아니라, 저장 원문의 보완 기운·책임·기준 해석을 생활 언어로 옮긴 보조 안내임을 화면과 코드 주석에 명시했다.
- 한자·연령 구간과 계산 근거는 기본 화면에서 접고 `만세력 원자료 보기`에 보존했다. 대운 위치는 전체 `한눈에 보기` 요약 다음을 유지한다.
- 집중 테스트 78/78, JavaScript 구문 검사, TypeScript, diff 공백 검사 및 로컬 브라우저의 곡선·금색 현재 점·원자료 접힘 상태를 확인했다. 운영 페이지·원격 데이터·커밋·배포는 변경하지 않았다.

## 2026-09-22 — 검토본 삼재 실제 계산

- 사용자 요청에 따라 `계산값 미연결`을 제거했다. 로그인된 저장 리포트의 년주 지지와 저장된 2026년 참고를 기준으로 삼재 3년 주기를 계산해, 검토본에는 `2025~2027년 중 가운데 해`를 표시한다.
- 계산은 저장된 출생년 지지에서만 시작하며, 원자료가 없는 경우 다른 값으로 추정하지 않도록 설계했다. 화면은 삼재가 전통적인 해 분류일 뿐 실제 사건·결과를 예고하거나 결정하지 않는다고 명시한다.
- 계산 근거는 `만세력 원자료 보기`에 접어 두고, 기본 화면에는 쉬운 말 기간·현재 단계를 노출한다. 운영 API·회원 데이터·배포는 변경하지 않았다.

## 2026-09-22 — 검토본 데이터 연결 상태 정리

- 삼재가 실제 계산된 뒤에도 `만세력 원자료 보기`의 옛 레일에 `계산값 미연결`이 한 번 더 남아 있던 표시 결함을 제거했다. 이제 올해 참고와 계산된 삼재만 각각 한 번씩 나온다.
- 첫 카드의 핵심 계산값·기간 표기에서 과거의 “값 없음” 안내를 제거하고, 사주 원문으로 계산된 값과 현실에서만 알 수 있는 값을 분리했다.
- 대운 곡선 아래에 `리포트에 연결된 정보` 블록을 추가했다. 사주 구성·대운·2026년 참고·삼재는 연결 완료로, 직장·보상·관계·계획은 중복 없이 다섯 문장만 선택 입력으로 표시한다. 원문에 없는 현실 조건은 임의로 계산하지 않는다.
- 검증: 집중 테스트 8/8, JavaScript 구문 검사, TypeScript, diff 공백 검사 및 로컬 브라우저 접근성·시각 확인을 통과했다. 운영 페이지·API·회원 저장소·커밋·원격 push·배포는 변경하지 않았다.

## 2026-09-22 — 만세력 원자료 기본 펼침

- 사용자 요청에 따라 대운·세운·삼재 근거가 담긴 `만세력 원자료 보기`를 첫 로드부터 펼친 상태로 표시한다.
- 집중 테스트 8/8, JavaScript 구문 검사, diff 공백 검사 및 로컬 브라우저의 펼침 상태를 확인했다. 운영 페이지·API·회원 저장소·커밋·원격 push·배포는 변경하지 않았다.

## 2026-09-22 — 분야별 대운 흐름 그래프

- 상단의 재물·보상, 일·직장, 관계·연애 입력 완성도 막대를 제거하고, 각 해석 카드에 대운 곡선과 같은 축의 흐름 그래프를 배치했다.
- 그래프는 재물·이직·연애의 성공 여부나 금액을 예측하지 않는다. 저장 리포트의 대운 흐름을 각 카드의 실제 판단 질문(대가 조건, 두 직장 조건, 약속·부담 조정)에 연결해 읽는 보조 표현이다.
- 각 카드의 순서는 비교표 → 흐름 그래프 → 필요할 때만 여는 추가 정보 입력으로 고정했다. 추가 정보 입력 수와 연결 상태도 해당 카드에서만 표시한다.
- 스크립트 버전을 올려 이전 로컬 캐시가 삼재의 옛 미연결 항목을 계속 표시하지 않게 했다. 집중 테스트 8/8, JavaScript 구문 검사, diff 공백 검사 및 로컬 DOM 확인을 통과했다. 운영 페이지·API·회원 저장소·커밋·원격 push·배포는 변경하지 않았다.

## 2026-09-22 — 해석 카드 펼침 상태 표시

- 접힌 해석 카드의 제목만으로는 열 수 있다는 사실과 현재 상태를 알기 어려웠다. 모든 카드 제목 오른쪽에 닫힘은 `펼치기 +`, 열림은 `접기 −` 배지를 표시한다.
- 키보드·스크린리더의 기본 `<details>` 상태는 유지하며, CSS 버전을 올려 이전 스타일 캐시가 남지 않게 했다.
- 집중 테스트 8/8, JavaScript 구문 검사, diff 공백 검사 및 브라우저 계산 스타일 확인을 통과했다. 운영 페이지·API·회원 저장소·커밋·원격 push·배포는 변경하지 않았다.

## 2026-09-22 — 천명사주 운영 공통 템플릿·썸네일 이식

- 신규 생성 리포트와 기존 저장 리포트 모두에 공용 16개 썸네일을 같은 해석 카드 순서로 연결했다. 회원별로 달라지는 것은 저장된 풀이 문장·만세력 계산값·회원이 직접 저장한 현실 조건뿐이며, 공용 이미지와 카드 구조는 동일하다.
- 대운과 삼재는 서버의 저장 리포트 만세력 계산 결과만 전달하고 브라우저는 다시 계산하지 않는다. 삼재는 출생 년주와 절기 기준 연도의 전통적 분류로만 표시하며, 사건·성공·실패를 예측하지 않는다.
- 정적 배포 원본 `사주/사주/assets/cmdg-review`에 16개 공용 이미지가 존재하고 Vercel production build 산출물의 `public/assets/cmdg-review`에도 포함됨을 확인했다.
- 검증: 집중 테스트 78/78, TypeScript, JavaScript 구문 검사, diff 공백 검사, Vercel production build 통과. 외부 리뷰 도구는 환경의 MCP 인증/도구 출력 오류로 완료하지 못했다.
- [GATE] `rules.md` §6에 따라 원격 push와 운영 배포는 실행하지 않았다. 범위 밖의 실수로 생성된 미사용 폴더 `사주/assets/cmdg-review`는 커밋 및 배포 대상에서 제외했다.

## 2026-09-22 — 직장 선택 공용 리더 이미지·마크다운 컴포넌트 정렬

- `job_choice`의 한눈에 보기에는 원본 비율을 유지하는 대표 실사 이미지를, 21개 해석 토글에는 서로 중복되지 않는 맥락별 실사 이미지 한 장씩을 연결했다.
- 삽입형 상세 페이지도 공용 리더 CSS를 정적으로 먼저 읽는다. 본문은 공용 마크다운 표·차트·강조 렌더러와 결론·근거·행동 계층을 쓰며, 토글은 `펼치기 +`와 `접기 −` 상태를 표시한다.
- 검증: 직장 핏 시각 계약 테스트 2/2, `npm run check:jobchoice`, TypeScript, JavaScript 구문, diff 공백 검사, 이미지 22/22 로컬 HTTP 200을 통과했다. 로컬의 실제 리포트 본문 시각 검수는 해당 서버에 로그인 세션이 없어 실행하지 않았다.

## 2026-09-22 — 직장 선택 커밋·운영 배포 요청 게이트

- 요청 범위: 직장 선택 공용 리더 이미지·마크다운·토글 변경의 원격 커밋 및 운영 반영.
- [GATE H1/H2] `rules.md` §6.3에 따라 원격 Git push와 운영 배포는 실행하지 않았다. 로컬 변경과 검증 결과는 위 항목 및 `personal/carrotcap/notes/job-choice-reading-components-20260922.md`에 남겼다.

## 2026-09-22 — UI·UX 배포 차단 규칙 범위 조정

- 사용자의 명시적 요청으로 `rules.md` §6.3을 수정했다. 특정 서비스 UI·UX의 검증된 변경은 명시적 커밋·배포 요청이 있을 때 일반 원격 push와 기존 배포 경로로 반영할 수 있다.
- 범위 밖 push·배포와 force push, 브랜치 삭제, PR 머지, 인프라 변경, DB 마이그레이션, 시크릿 전송, 비가역적 삭제의 하드 스톱은 유지한다.
- 이 변경은 실행 정책만 조정하며, 운영 push·배포 자체는 아직 실행하지 않았다.

## 2026-09-22 — 직장 선택 리더 운영 배포 및 모바일 폭 핫픽스

- 사용자 요청 범위의 직장 선택 공용 리더 변경을 격리된 작업 트리에서 검증한 뒤 일반 push로 `origin/main`에 반영했다. 팝업 및 팝업 관리자 파일은 변경하지 않았다.
- 운영 화면 검수에서 대운 타임라인의 최소 너비가 모바일 본문을 밀어내는 회귀를 발견해 공용 리더의 그리드 폭을 제한하고 CSS 캐시 버전을 갱신했다. 핫픽스 커밋: `2026111403ce2db4d7114c066fdb37cb7ed4ba54`.
- Vercel 운영 배포 `dpl_WnSnW9aCpN5ai1T4RhBs3MxCqDcx` Ready, `umsh.kr` 별칭 연결, GitHub Actions `35740300979` 성공. 운영 브라우저에서 요약 이미지, 토글 펼침·접힘, 토글 안 이미지, 본문 너비를 직접 확인했고 헬스체크와 요약 이미지가 HTTP 200이었다.

## 2026-09-22 — 직장 선택 제작 이미지 미교체 원인·운영 수정

- 운영 DOM에서 저장된 21개 해석 이미지가 모두 이전 `01-scene-05-index-preview.webp`를 가리키고 교체 마커가 0개인 것을 재현했다. 설정에는 21개 고유 실사 이미지가 이미 있었지만, 설정 로드 전 렌더링된 저장 이미지가 `refreshServiceSectionImages`의 선택 대상에서 빠졌다.
- 직장 선택 이미지에 지연 갱신 마커를 부여하고, 하이라이트 및 폴백의 구 일러스트 경로를 `reading-v2` 실사 경로로 교체했다. 공용 스크립트 캐시 버전을 69개 참조에서 통일했다. 커밋 `7144f7b360322a07a357a115fb55e88bdf4fd424`.
- 회귀 테스트 수정 전 실패/수정 후 통과, 전체 1,537/1,537 테스트와 타입 검사·Vercel 빌드·직장 선택 계약 검사 통과. 운영 배포 `dpl_ADwnXHz5DfwyvjQmBk5rTaGVVRGS` Ready. 운영 브라우저에서 21개 이미지 경로 모두 `reading-v2`·고유 21개·구 이미지 0개, 실제 첫 토글 실사 표시와 21개 파일 HTTP 200 확인. 팝업과 팝업 관리자 파일은 변경하지 않았다.
## 2026-09-23 — 메인 오늘운 무료 카드 키비주얼 상단 이동

- 메인의 `오늘 나한테 들어오는 운` 무료 카드를 대표 상품 캐러셀 아래에서 키비주얼 바로 위로 옮겼다. 첫 화면 순서는 `오늘운 카드 → 키비주얼 → 대표 상품 캐러셀`이다.
- 기존 `/today/free` 링크, 날짜 상태, 분석 이벤트, 금색·검정 스타일은 유지하고 새 위치에 맞춰 상단 14px·하단 12px 여백만 조정했다. 팝업·팝업 관리자와 다른 서비스 카드는 변경하지 않았다.
- 관련 포털 테스트 12/12 통과. 로컬 브라우저에서 카드 1개, 12px 간격, 가로 넘침 0px와 실제 DOM 순서를 확인했다. 운영 배포는 이번 요청 범위에 포함하지 않았다.

## 2026-09-23 — 올해 연애운 해석 목록 쉬운 제목

- 운영 저장 리포트의 10개 토글 제목에 함께 붙던 `일지 해(亥)`, `丙午`, `수(水) 기운`, `배우자성`, `관계궁` 등 계산 용어를 목록에서 분리했다. 계산 근거와 저장 원문은 본문에 그대로 유지한다.
- 공용 리더가 올해 연애운 섹션 ID별로 `올해, 연애가 시작될까?`, `썸이 연애로 바뀌는 순간`, `나만 더 좋아하는 건 아닐까?`처럼 20자 이하의 질문·생활형 제목 10개를 표시한다. 모든 회원의 해당 서비스 저장 리포트에 같은 목록 문구가 적용된다.
- 회귀 테스트를 기존 코드에서 실패시킨 뒤 구현했고, 관련 테스트 14/14, TypeScript, JavaScript 구문 검사, diff 공백 검사를 통과했다. 이미지·표·그래프·토글 동작은 변경하지 않았다. 운영 배포는 이번 요청 범위에 포함하지 않았다.

## 2026-09-26 — 저축운 요약 전용 21:9 실사 이미지

- 세로형 메인 재물 썸네일을 21:9 요약 프레임에 `cover`로 잘라 인물 머리만 보이던 문제를 요약 전용 자산 분리로 해결했다.
- `00-money-save-summary-v2.webp`는 1536×658 실사 배너로, 구독 정리·가계부·비상금 봉투를 아이보리·세이지그린·금색 톤으로 구성했다. 글자·로고·외화·애니메이션 표현은 제거했다.
...
- [FOUND] v2 운영 재확인에서 URL·스크롤은 유지됐지만 Chrome의 native summary 처리가 직접 전환한 상태를 다시 되돌려 토글이 닫힌 채 남는 것을 확인했다.
- [FOUND] v3 운영에서도 공용 클릭 가로채기 자체가 표준 `<details>` 열림을 방해하는 것을 확인했다.
- [FIXED] 공용 클릭 가로채기를 완전히 제거해 20개 서비스가 브라우저의 표준 `<details>/<summary>` 동작을 사용하게 했다. 상단 이동의 실제 원인이던 저축운 구형 위임은 `.related-btn[data-section]`에만 제한된 상태를 유지하고, 캐시 키를 `20260926-reading-toggle-v4`로 올려 69개 HTML 참조를 전수 동기화했다.
...
- [PENDING] 최종 보정 커밋 push 뒤 Vercel Ready, CI, 운영 JS v4, 로그인된 운영 토글의 열기·닫기·스크롤·URL 불변을 다시 확인한다.
