...
## 2026-09-22 — 올해 연애운 목차별 실사형 이미지 보강

### 대표 썸네일 보강

- `06-step-6_1-report-detail`의 `#hero-image`를 동일한 실사형 창가 표지로 교체했다.
- 초기 HTML과 상세 렌더링 모두 같은 자산을 사용하도록 고정해 화면 전환 시 이전 일러스트로 되돌아가지 않는다.

- `love_this_year`의 10개 저장 해석 목차에 서로 다른 실사형 장면을 연결했다. 창가·소개·모임·월별 계획·관계 속도·신호 확인·궁합 비교·대화 온도·첫 행동으로 장면을 분리하면서, 사용자 제공 참조 이미지의 자연광·아이보리·연핑크 톤을 유지했다.
- 공용 리더는 서비스가 `sectionImages`를 명시할 경우 저장된 기존 이미지를 대신해 목차별 편집 이미지를 우선한다. 다른 서비스는 기존 저장 전용 이미지와 A/B 보조 이미지 우선순위를 유지한다.
- 팝업 관리자·팝업, 저장 리포트 원문·개인 계산값·DB는 변경하지 않았다. 공개 자산 10개는 로컬 HTTP에서 모두 200 `image/png`을 확인했다.
- 검증: `node --check`, 목차별 이미지 계약 테스트 4/4, 관련 하이라이트 테스트 14/14, TypeScript 및 Vercel build, diff check PASS. [GATE] 커밋·원격 push·운영 배포는 이번 요청에 명시되지 않아 미실행이다.
...
## 2026-09-22 — 서비스 공통 읽기 형식·올해 연애 신호 그래프·도화 실사 배너 (로컬 검토)

- 공용 유료 리더의 첫 목차에 저장 사주의 실제 오행 5개 계산값을 막대그래프로 표시한다. 성공률·예측 점수가 아님을 명시하고 서비스별 색·제목·기존 이미지 구성을 유지한다. 저장 원문은 수정하지 않는다.
- 공용 안전 마크다운 뷰에 인용·기울임·밑줄·형광 강조를 추가했고, 집 풍수·결혼 날짜 전용 상세도 이를 재사용한다. HTML 원문은 계속 이스케이프한다.
- 앞으로 생성될 일반 유료 해석 항목의 목표 분량을 1200~1600자로 높였다. 기존 회원의 저장 본문을 임의로 늘리거나 재생성하지 않는다.
- 올해 연애운의 월별 흐름에는 로그인 리포트의 출생값·저장 연도로 계산한 3개 전통 신호의 12개월 개수(0~3)를 표시한다. 저장된 장식용 퍼센트 차트 2곳은 가리고, 만남·연애 성공률로 해석하지 않도록 설명한다.
- 첫 하이라이트 ‘도화가 들어오는 달’은 새 실사형 겨울 카페 배너로 교체하고 얕은 배너 영역의 얼굴 크롭을 조정했다. 기존 10개 목차 이미지와 대표 썸네일은 유지한다.
- 검증: `npm test` 1529/1529, 변경 후 집중 테스트 12/12, TypeScript, `qa:all-services` 20/20, Vercel build, `git diff --check` 통과. 공개 결제·팝업 관리자·팝업은 변경하지 않았다.
- 한계: 집 풍수·결혼 날짜의 전용 레이아웃은 이번에 공용 마크다운만 공유한다. 신규 생성 분량은 기존 저장 리포트에는 소급되지 않는다. 로그인 고객의 운영 시각 검수와 원격 배포는 별도 요청 전 미실행이다.

## 2026-09-22 — 올해 연애운 한눈에 보기 전용 가로 이미지 (로컬 검토)

- 세로형 `01-frontface-window-poster.webp`를 가로 요약 카드의 썸네일로 재활용해 발생한 얼굴 크롭을 제거했다.
- 같은 창가·아이보리·연핑크·뮤트 라벤더의 서비스 아트 디렉션으로 새 3:2 가로 실사 이미지를 생성해 `00-year-love-summary-wide-v1.png`으로 연결했다.
- `summaryImageFit: wide`는 이 자산만 원본 비율 그대로 표시하고 오버레이를 제거한다. 따라서 가로 카드에서 인물의 머리·손·휴대폰이 잘리지 않는다.
- 검증: 이미지 자산·설정·렌더러·CSS 계약 13/13, `node --check`, `git diff --check`, Vercel build 통과. 운영 배포와 로그인 페이지 실화면 확인은 미실행이다.

## 2026-09-22 — 올해 연애운 06 공통 리더 상·하단 정렬 (로컬 검토)

- `올해 연애운` 상세의 별도 상태/PDF 바와 채팅 입력창은 공용 GNB·하단 메뉴와 중복되므로 인증된 저장 해석에서는 보이지 않게 했다. 정적 폴백 이벤트 핸들러는 제거하지 않았다.
- 모든 표준 in-place 06 상세에는 공통 리더 액션 영역을 붙인다. 리포트 끝에서 `링크 공유하기`는 canonical `/r/:id`만 클립보드로 복사하고, `PDF 저장`은 기존 인쇄 경로를 사용한다. 링크 복사는 열람 권한을 양도하지 않는다.
- 공용 접근 스크립트와 in-place CSS 캐시 버전을 69개 HTML 참조에 맞췄다. 팝업 관리자·팝업, DB, 저장 리포트 원문·개인 계산값은 변경하지 않았다.
- 검증: 로컬 정적 화면에서 공통 상단·하단만 남는 것을 시각 확인했고, 집중 회귀 93/93, 전체 `npm test`, TypeScript, Vercel build, `git diff --check`를 통과했다.
- [GATE] 이번 요청에는 배포 지시가 없으므로 원격 push·운영 배포는 실행하지 않았다. 로그인된 실제 리포트에서의 액션 버튼 클릭 검수는 배포 후 별도로 수행한다.

## 2026-09-22 — 팝업 관리자 버튼 클릭 집계 (로컬 검토)

- 팝업 관리자 표에 각 게시 팝업의 `최근 30일` 실제 버튼 클릭 수를 표시한다. 기존 퍼널 집계 API만 읽으며, 조회 실패 시에는 0으로 위장하지 않고 `집계 불가`를 표시한다.
- 새 클릭 이벤트는 팝업 콘텐츠의 불변 ID를 `signup_popup:<id>`로 전송한다. 기존 GA4 액션 이름과 팝업 노출·숨김 동작은 유지했다.
- 첫 페이지 포털에도 공용 퍼널 수집기를 로드했고, 공용 크롬에서는 수집기를 팝업보다 먼저 요청해 첫 클릭 유실 가능성을 줄였다.
- 이전에는 공통 CTA 이름만 남았으므로 과거 이벤트를 특정 팝업 버전으로 나누지 않는다. 팝업별 정확한 값은 배포 시점 이후부터 누적된다.
- 검증: 집중 단위 테스트 55/55, TypeScript, Vercel build, `git diff --check` 통과. CreamWIKI `personal/carrotcap/notes/umsh-popup-manager-cta-click-metrics-20260922.md`에 결정·한계를 기록했다. [GATE] 커밋·원격 배포는 아직 실행하지 않았다.

## 2026-09-22 — 여덟 서비스 대표 이미지 단일화 (배포 전)

- 사용자 최신 결정에 따라 직장 선택·퇴사운·고양이 궁합·이직운·소비성향·결혼궁합·애인 바람 신호·커플궁합은 각 저장 해석의 상단 대표 썸네일 한 장만 쓴다. 토글·하이라이트에 같은 이미지나 추가 생성 이미지를 반복하지 않는다.
- `sectionImageMode: summary-only`를 `longform-blocks.json`의 해당 여덟 서비스에만 계약으로 추가했다. 공용 리더는 설정이 도착하기 전에도 같은 키를 즉시 판별해 본문 카드 그림·하이라이트 배너·정적 항목 비주얼을 생략한다. 저장된 원문·목차·개인별 계산값·권한은 그대로 유지한다.
- 최초 토글의 안전 마크다운, 3행 읽기 표, 실제 오행 계산 막대그래프, 명시적 펼치기/접기 표시는 공용 렌더러를 통해 계속 제공한다. 예측 점수·미연결 안내문·임의 본문은 추가하지 않는다.
- 검증: 집중/회귀 131/131, 전체 `npm test`, TypeScript, Vercel build, 20개 서비스 계약 QA, 20개 서비스 정적 QA, `git diff --check` 통과. 배포 후 로그인된 저장 리포트의 구조와 단일 이미지 수를 확인한다.
...
## 2026-09-22 — 삽입형 대운 흐름 공통 가독성 보강 (로컬 검토)

- 관측: 올해 연애운을 포함한 삽입형 06 상세에서 대운 컴포넌트가 동적 CSS 주입에만 의존했다. CSS가 늦게 오거나 이전 캐시가 남으면 제목·그래프·원자료가 기본 브라우저 서식으로 붙어 읽기 어렵게 보인다.
- 결정: 천명사주 공용 리더의 읽기 위계를 기준으로 19개 삽입형 상세 문서가 `umsh-verified-inplace.css`를 정적으로 먼저 읽도록 했다. 런타임 CSS 주입은 폴백으로 유지한다.
- 표현: 만세력 레이블·제목·현재 위치를 분리하고, 현재 위치는 정보 칩으로, 설명은 그래프 위 안내문으로, 그래프는 색상 토큰과 한국어 폰트로 표시한다. 원자료 카드·토글·삼재/올해 참고 배치도 공용 컴포넌트로 유지한다.
- 검증: 19개 상세의 정적 스타일 연결 계약 13/13, 전체 `npm test`, TypeScript, `check:jobchoice`, 20개 서비스 QA, Vercel build, diff check PASS. 운영 로그인 화면 시각 검수와 배포는 미실행이다.
- [GATE H2] 원격 push·운영 배포는 `rules.md` §6.3에 따라 이번 로컬 검토 변경에서는 실행하지 않았다.

## 2026-09-22 — 공통 리포트 공유·PDF 버튼 가로 배치

- 원인: 공통 리더가 공유 버튼과 PDF 버튼 사이에 상태 메시지를 렌더링하고, 해당 메시지가 그리드 두 칸을 차지해 PDF가 다음 줄로 밀렸다.
- 수정: 공통 하단 도구에서 공유·PDF를 첫 줄 좌우에 명시적으로 배치하고 상태 메시지만 둘째 줄에 배치했다. 좁은 화면에서도 같은 순서를 유지한다. 19개 삽입형 상세의 CSS 버전을 갱신해 이전 캐시를 피했다.
- 범위: 스타일, CSS 참조 버전, 계약 테스트만 변경했다. 리포트 원문·개인정보·팝업·팝업 관리자는 변경하지 않았다.
- 검증: 공통 리더 계약 13/13, 전체 단위 테스트 1537/1537, TypeScript, diff check 통과. 운영 배포와 실화면 검증 결과는 배포 후 별도로 확인한다.

## 2026-09-23 — 결혼궁합 공용 저장 리포트 화면 보강 (로컬 검토)

- 운영 기존 DOM에는 저장 해석 24/24가 있었지만 `.phone` 최대 높이와 공용 셸의 `overflow: clip`으로 후반 항목이 가려졌다. 결혼궁합 06 상세만 문서 전체 스크롤로 바꿨다.
- 최신 사용자 요청에 따라 기존 결혼궁합 `summary-only` 계약을 해제하고 요약·첫 하이라이트·24개 해석에 각기 다른 실사 WebP 26장을 연결했다. 다른 서비스의 이미지 정책과 저장 원문·개인별 계산값은 유지했다.
- 천명사주의 글 위계를 결혼궁합의 로즈·와인 팔레트로 적용했다. 토글 상태·한 줄 답 강조·15px 본문·표/그래프의 밝은 글자 대비를 보강하고 중복 상담 입력을 숨겼다. 공유·PDF 공통 도구는 유지했다.
- 공용 JS/CSS 참조를 69개 HTML에서 단일 새 버전으로 맞춰 이전 캐시 혼용을 막았다. 데이터 설정 JSON 버전도 갱신했다.
...
## 2026-09-23 — 고양이 궁합 20개 저장 해석 시각 계약 (배포 전 검증)

- 운영 저장 화면의 완료된 해석 20개와 기존 대표 이미지 한 장, 카드 이미지 0장을 확인했다. 실제 저장 원문·개인별 계산값·결제/소유권 로직은 변경하지 않았다.
- 메인 썸네일의 검정·금색·연두 톤을 실사 생활 사진으로 옮겨 요약·하이라이트·20개 목차에 중복 없는 WebP 22장을 연결했다. 각 사진은 해당 목차의 손길·거리·루틴·공간·놀이·휴식·건강·장기 돌봄 맥락을 따른다.
- 고양이 궁합만 대표 한 장 모드에서 제외했다. 공용 저장 리더가 편집 설정을 받은 후 목차 이미지를 표시하고 구형 저장 컷도 교체한다. 06 상세는 본문 전체 스크롤, 15px 글·1.9 행간, `펼치기 +`/`접기 −`, 3:2 사진 비율을 사용한다. 저장 본문이 있을 때만 하단의 중복 결제 안내를 숨긴다.
- 공용 JS 참조 버전을 전 페이지에서 통일했다. 사진 22개는 SHA-256 22개가 서로 다르며 1,204,196바이트다. 집중 테스트 17/17, `check:cat`, 전체 단위 테스트 1,539/1,539, TypeScript, 서비스 QA 20/20, Vercel build, `git diff --check` 통과. 운영 배포와 실브라우저 사진 확인 결과는 별도로 기록한다.
- 범위: 고양이 궁합 UI·공용 리더 캐시 참조만. 팝업·팝업 관리자·DB·고객 저장 해석 변경 없음. 복구 기준은 배포 직전 Ready 버전이다.
- 로컬 정적 HTTP에서 새 WebP 22/22가 200이고 06 상세의 `max-height: none`, `overflow: visible`, 새 스크립트 버전을 확인했다. 회원 API가 없는 로컬 정적 화면에는 실데이터를 흉내 내지 않았다.
- Grok 리뷰를 프로젝트 경로로 호출했지만 9분간 보고서 없이 대기해 중단했다. 리뷰 결과는 NOT_RUN이며 이를 승인으로 간주하지 않는다. 변경 목록·데이터 계약·전체 회귀 테스트를 직접 점검한 뒤 운영 실화면에서 다시 검수한다.

## 2026-09-23 — 공용 오행 계산값 표 가독성 수정

- 관측: 삽입형 06 상세에서 오행 계산값이 의미상 목록으로 렌더되고 전용 스타일은 전체화면 리더 범위에만 있어, 고양이 궁합 모바일 화면에서 `나무 5 불 0…`이 세로로 한 글자씩 깨졌다.
- 결정: 저장된 실제 오행 횟수는 유지하고, 공용 렌더러를 5열 시맨틱 표(`나무·불·흙·쇠·물` / 계산값)로 바꿨다. 작은 상대 막대는 수치 비교의 보조 표현으로만 남겼다.
- 적용: 전체화면·삽입형 리더 모두 표의 헤더·수치·경계·가로 오버플로 스타일을 갖는다. 공용 JS/CSS 캐시 버전을 리포트 화면 전체에서 통일해 새 마크업과 옛 스타일이 섞이지 않게 했다.
- 검증: 집중 테스트 15/15, 전체 단위 테스트 1,539/1,539, TypeScript, 고양이 계약 검사, diff check 통과. 운영 배포 후 로그인된 저장 리포트에서 실제 표와 좁은 화면을 다시 확인한다.

## 2026-09-23 — 공용 마크다운 뷰 표현 체계 보강

- 운영 고양이 궁합 화면을 직접 검사해 첫 토글의 읽기 순서 표가 시맨틱 표이지만 삽입형 리더 CSS 범위에서 빠져 일반 텍스트처럼 보이는 원인을 확인했다. 오행 표와 대운 그래프는 실제 계산값을 유지했다.
- 삽입형·전체화면 리더에 표, 목록, 소제목, 인용문, 굵게, 기울임, 밑줄, 형광 강조, 인라인 코드의 공통 위계와 서비스 포인트 색상 연결을 추가했다. 첫 토글 읽기 표도 헤더·행 경계·가로 오버플로를 갖는 표로 통일했다.
- 마크다운 표 헤더에 `scope="col"`을 붙이고, 가로 스크롤 영역에 레이블·키보드 포커스를 제공했다. 데이터가 없는 그래프나 점수는 새로 만들지 않았다.
- 공용 JS/CSS 캐시 버전을 화면 전체에서 통일했다. 집중 테스트 23/23, 전체 단위 테스트 1,540/1,540, TypeScript, `check:cat`, 서비스 QA 20/20, Vercel build 통과. 팝업·관리자·DB·저장 원문은 변경하지 않았다.

## 2026-09-23 — 이직운 저장 리포트 시각·가독성 보강

- 운영 저장 리포트에서 완료된 해석 10개, 읽기 순서 표, 오행 표, 실제 만세력 대운 그래프는 존재하지만 `work_move`가 대표 이미지 한 장 모드에 남아 토글 이미지가 0장인 상태를 확인했다.
- 메인 썸네일의 야간 오피스·검정·버건디·금색 톤을 실사형으로 재해석해 요약·하이라이트·10개 해석에 서로 다른 WebP 12장을 연결했다. 각 컷은 현재 자리 비교, 역할 경계, 직무 핏, 새 회사 적응, 계약 조건, 이직 시기, 브레이크, 90일 계획, 최종 결정의 실제 맥락을 따른다.
- 저장 원문과 회원 계산값은 변경하지 않았다. 공용 마크다운 뷰의 시맨틱 표·실제 계산 그래프를 유지하고, 이직운 상세에 15px 본문·1.9 행간, `펼치기 +`/`접기 −`, 3:2 이미지, 전체 문서 스크롤을 적용했다. 저장 리포트가 채워지면 하단의 구형 상세·상담 입력 중복 영역은 숨긴다.
- 팝업·팝업 관리자·DB·결제·소유권 로직은 변경하지 않았다. 운영 반영 뒤 해당 저장 리포트에서 10개 토글 이미지·표·그래프·공유/PDF·모바일 끝 스크롤을 다시 확인한다.
- Grok 리뷰는 실행했으나 외부 MCP 인증 경고 뒤 최종 판정 없이 멈춰 `NOT_RUN`으로 기록한다. 이를 승인으로 간주하지 않고 집중 회귀·전체 회귀·타입 검사·20개 서비스 QA·배포 빌드 결과로 직접 검증했다.

## 2026-09-23 — 이직운 후속 채팅 입력 비노출

- 저장 이직운 상세 하단의 질문 입력 폼과 제출 이벤트를 함께 주석 처리했다. 화면만 감추지 않고 입력·임시 저장 동작도 중지했으며, 향후 기능 재검토 시 함께 복원할 범위를 코드 주석으로 남겼다.
- 저장 해석·공용 리포트 도구·팝업·팝업 관리자·DB는 변경하지 않았다. 이직운 리포트 계약 테스트 16/16과 `git diff --check`를 통과했다.

## 2026-09-23 — 커플궁합 28개 저장 해석 시각·가독성 보강

- 커플궁합의 실제 28개 목차를 확인하고 대표 이미지 한 장 모드를 해제했다. 기존 대표 썸네일·하이라이트는 유지하며, `케미 한 줄`부터 `한 문장 확인 질문`까지 각 목차에 서로 다른 3:2 실사형 WebP 한 장씩을 연결했다.
- 서비스 키 별칭 `couple_match`를 `match_couple`로 정규화하고 저장된 구형 이미지도 설정 도착 후 새 목차 이미지로 교체한다. 저장 원문·회원 계산값·시맨틱 표·실제 대운 그래프는 변경하지 않았고 임의 점수는 만들지 않았다.
- 공용 리더가 채워지면 구형 rail·단일 상세 화면을 숨긴다. 본문 15px/1.9 행간, 결론 16px/1.65, `펼치기 +`/`접기 −`, 3:2 이미지, 전체 문서 스크롤을 적용했다.
- 이미지 28/28 고유 해시, 집중 테스트 19/19, 전체 단위 테스트 1,542/1,542, TypeScript, 20개 서비스 QA, Vercel build, `check:couple`, diff check 통과. 운영 배포와 로그인된 변경 후 시각 검수는 미실행이다.

## 2026-09-23 — 퇴사운 20개 저장 해석 시각·가독성 보강

- 운영 저장 리포트의 20/20 해석, 시맨틱 읽기 표, 오행 계산 표, 회원별 대운 그래프가 정상 저장된 것을 확인했다. `quit_fortune`의 대표 이미지 한 장 설정만 목차 이미지 표시를 막고 있었다.
- 대표 이미지 한 장 모드를 해제하고 메인 썸네일의 검정·청록·금색 톤을 따른 3:2 실사형 WebP 20장을 각 해석에 한 장씩 연결했다. 이미지 20장은 1536×1024이며 SHA-256 해시도 20개가 모두 다르다.
- 저장 원문·회원별 계산값·공용 마크다운 표·실제 대운 그래프는 유지했다. 임의 점수와 예측 그래프는 만들지 않았다.
- 퇴사운 상세에 15px 본문·1.9 행간, 결론 16px·1.65 행간, `펼치기 +`/`접기 −`, 전체 문서 스크롤을 적용했다. 저장 데이터가 채워진 경우에만 구형 상세·상담 입력 중복 영역을 숨긴다.
- 집중 테스트 18/18, `check:quit`, 전체 단위 테스트 1,543/1,543, TypeScript, 서비스 QA 20/20, Vercel build, `git diff --check`를 통과했다. 운영 배포와 배포 후 로그인 실화면 검수는 이번 요청 범위에 포함하지 않았다.
# 2026-09-23 운영 UI 보강 배포 준비

- 메인 무료 오늘운 카드를 키비주얼 바로 위로 이동하고 기존 링크·팝업 추적 구조를 유지했다.
- 올해 연애운 저장 리포트의 10개 토글 제목을 중학생도 바로 이해할 질문형 문구로 바꿨다.
- 기존 이직·퇴사·커플 저장 리포트의 이미지·표·토글 개선 커밋과 함께 운영 기준 브랜치에서 검증한다.
- 팝업과 팝업 관리자 경로는 변경하지 않는다.
- Grok 코드 리뷰는 실행했으나 로컬 Grok의 외부 MCP 인증·초기화 오류로 완료되지 않아 `NOT_RUN`으로 기록한다. 대신 전체 1,546개 단위 테스트, 20개 서비스 QA, TypeScript, Vercel 빌드 및 배포 경로 차이 검사를 통과했다.

## 2026-09-23 — 이직운 공용 리더 아래 구형 본문 중복 제거

- 운영 이직운 저장 리포트에서 공용 리더의 10개 해석·고유 이미지·마크다운 표·대운 그래프는 정상 렌더링됐지만, 공용 리더 아래에 구형 결론·근거·상세·행동·주의·연관 항목이 다시 노출되는 문제를 확인했다.
- 런타임이 `#detail-stack`을 `#content` 안으로 이동해 형제 선택자 기반 비노출 규칙이 더 이상 맞지 않는 것이 원인이었다.
- 구형 노드에 공용 `data-umsh-legacy-reading-ui` 표식을 부여해 저장 데이터가 채워진 뒤에만 숨기고, 대표 화면·진행률·공용 리더·공유/PDF는 그대로 유지했다.
- 전체 단위 테스트 1,546/1,546, TypeScript, Vercel build, `git diff --check`를 통과했다. 팝업·팝업 관리자·DB·저장 원문은 변경하지 않았다.

## 2026-09-23 — 공용 마크다운 번호 목록 연속 표시

- 이직운 하이라이트의 체크리스트가 설명 문단마다 별도 `<ol>`로 렌더링되어 모든 번호가 `1.`로 다시 시작하는 문제를 재현했다.
- 저장 해석 원문은 바꾸지 않고, 같은 해석 블록 안에서는 분리된 번호 목록의 `start` 값을 이어 주도록 공용 마크다운 렌더러를 수정했다. 서로 다른 해석 블록에서는 다시 1부터 시작한다.
- 요약·하이라이트·토글 본문이 모두 같은 목록 상태 헬퍼를 사용하도록 통일하고, 상세 리포트의 공용 스크립트 캐시 버전을 함께 갱신했다.
- 재현 테스트는 수정 전 실패, 수정 후 통과했다. 집중 테스트 11/11, TypeScript, Vercel build를 통과했으며 팝업·팝업 관리자·DB·저장 원문은 변경하지 않았다.

## 2026-09-23 — 이직운 중복 전체 목록 버튼 제거

- 이직운 상세 화면이 이미 전체 해석 목록을 포함하는데 하단에 `전체 목록으로 돌아가기`가 다시 노출되는 중복 탐색을 확인했다.
- 해당 링크와 사용되지 않는 전용 CSS만 제거했다. 공용 링크 공유·PDF 저장 버튼, 해석·이미지·표·그래프는 유지했다.
- 재발 방지 정적 테스트를 추가했고 집중 테스트 18/18과 TypeScript 검사를 통과했다. 팝업·팝업 관리자·DB·저장 원문은 변경하지 않았다.

## 2026-09-23 — 보관함 오늘운 월간 출석 달력

- 보관함 `오늘 운` 탭 상단에 이전·다음 달을 이동할 수 있는 월간 달력을 추가했다. 서버에 저장된 오늘운 날짜만 출석 도장으로 표시하며, 도장이 찍힌 날을 누르면 해당 날짜의 저장 리포트를 연다.
- 달력 아래 오늘운 목록은 최신 5건만 표시한다. `구매한 운` 탭의 전체 목록, 실제 저장 데이터, 관리자 삭제, PDF 동작은 유지했다.
- 한 달 최대 31일이 다른 구매 리포트에 밀려 잘리지 않도록 본문 없는 경량 목록 한도를 30건에서 서버 조회 한도와 같은 100건으로 맞췄다. 해석 원문과 DB 구조는 변경하지 않았다.
- 달력·링크·최신 5건·탭 격리를 고정하는 단위 테스트를 추가했다. 집중 테스트 18/18, 전체 단위 테스트 1,552/1,552, TypeScript, Vercel build, `git diff --check`를 통과했다.
- Grok 코드 리뷰는 외부 MCP 인증 및 파일 읽기 오류로 완료되지 않아 `NOT_RUN`으로 기록한다. 로컬 보관함은 로그인 상태가 없어 데이터가 채워진 시각 검수 대신 DOM/CSS 계약과 전체 회귀 검증을 수행했다.

## 2026-09-23 — 커플 시그널 21개 저장 해석 시각·가독성 보강

- `couple_signal`에 남아 있던 대표 이미지 한 장 모드를 해제하고, 실제 목차 순서 21개에 맞춘 3:2 실사형 WebP를 각 해석에 정확히 한 장씩 연결했다. 메인 썸네일의 검정·버건디·골드 톤은 유지하되 신뢰, 거리감, 외부 경계, 생활 애정, 갈등, 불안, 확인 대화, 안전한 거리 두기의 맥락을 컷마다 구분했다.
- 저장 원문과 회원별 계산값은 변경하지 않았다. 공용 리더의 시맨틱 읽기 표·오행 계산 표·실제 만세력 대운 그래프만 그대로 재사용하고 임의 점수나 가짜 그래프는 만들지 않았다.
- 상세 화면은 448px 공용 리더 폭, 15px 본문·1.9 행간, 결론 16px·1.65 행간, `펼치기 +`/`접기 −`, 3:2 이미지와 전체 문서 스크롤을 사용한다. 저장 리포트가 채워진 경우에만 구형 목록 바와 이전·다음 바를 숨긴다.
- 운영 저장 리포트 원문은 비로그인 API에서 401로 보호되어 직접 재조회하지 않았다. 기존 10개 대분류·21개 항목 계약과 공용 렌더러를 기준으로 구현했고, 로컬 브라우저에서는 비로그인 잠금 상태가 개인정보를 노출하지 않는 것까지 확인했다.
- 이미지 21장 시각 샘플 검수, 집중 테스트 21/21, 전체 단위 테스트 1,553/1,553, TypeScript, `check:signal`, 20개 서비스 QA, Vercel build, `git diff --check`를 통과했다. 팝업·팝업 관리자·DB·결제는 변경하지 않았다.

## 2026-09-23 — 저축운 16개 저장 해석 이미지·가독성 보강

- `money_save`의 8개 대분류·16개 실제 목차를 기준으로 대표 이미지 한 장 모드를 해제했다. 메인 재물 썸네일의 초록 머리 고양이 귀 안내자, 짙은 에메랄드·금색·수채화 웹툰 톤을 유지한 3:2 WebP 16장을 각 해석에 한 장씩 연결했다.
- 저장 원문과 회원별 계산값은 변경하지 않았다. 공용 리더의 시맨틱 읽기 표·실제 오행 계산 표·회원별 대운 그래프만 그대로 재사용하고 임의 점수나 예측 그래프는 만들지 않았다.
- 상세 화면에 15px 본문·1.9 행간, `펼치기 +`/`접기 −`, 전체 문서 스크롤을 적용했다. 밝은 종이 배경에서 본문·보조문·표 머리글·대운 그래프 축의 대비를 높였고, 저장 리포트가 채워진 경우에만 구형 상세·이전/다음·중복 PDF UI를 숨긴다.
- 생성 이미지 16장은 모두 1536×1024이고 SHA-256 해시도 16개가 모두 다르다. 전체 단위 테스트 1,554/1,554, `check:save`, TypeScript, 20개 서비스 QA, Vercel build, `git diff --check`를 통과했다. 로컬 브라우저에서는 비로그인 잠금 상태가 저장 원문이나 개인정보를 노출하지 않는 것을 확인했다.
