...
export function buildLoveThisYearStoryBeat(
  ...
  const beats: Record<string, StoryBeat> = {
    'love-year-possibility': {
      hook,
      feel: `흠… ${f.name}님. "${f.concern}" 그 질문이 올해 연애 가능성을 여는 첫 장면이에요. ${f.relation} 상태에서도 마음은 이미 한 발 앞서 두리 있어요. 생길까 말까보다, 알아채고도 지나친 밤이 아쉽지 않은가가 더 절실하지.`,
      softBridge: softYear,
      tableMd: [
        '| 마음 신호 | 올해의 장면 |',
        '| --- | --- |',
      ...
      actions: [
        ...
        '혼자 확대해석한 밤은 짧게 접고, 실제 만남을 한 번 더 보세요',
      ],
      imagePrompt: {
    ...
    'love-attraction-pattern': {
      ...
      scene: f.hasPartner
        ? `${f.partnerName}(${f.partnerRelation}) 앞에서도 같은 패턴이 올라올 수 있어요. 좋아할수록 말이 줄거나, 반대로 확신을 너무 빨리 구하는 순간. 그 장면이 오면 관계가 흔들리는 게 아니라, ${f.name}의 오래된 끌림 습관이 깨어난 거예요.`
        : `아직 특정 상대가 없어도 패턴은 먼저 와. 자극이 센 사람에겐 금세 불이 붙고, 조용히 안정적인 사람 앞에서는 오히려 심장이 헷갈리네. 올해는 설렘의 크기보다, 그 설렘이 ${f.name}을 흐리게 하는지 선명하게 하는지를 보세요.`,
      actions: [
        '설렌 직후 24시간은 결론을 내리지 마세요',
        '"이 사람이 나를 편하게 하는가"를 한 줄로 적어 보세요',
    ...
    'love-dohwa-months': {
      hook,
      feel: `${f.dohwaMonth}. 그 시기엔 유난히 시선이 머물고, 평소보다 말이 길어져요. 도화는 인기가 폭발한다는 뜻이 아니라, 마음이 사람을 향해 문을 여는 공기예요.`,
      ...
      chartPoints: chart,
      chartCaption: '올해 연애 온도가 출렁이는 달들이에요.',
      scene: f.hasPartner
        ? `${f.partnerName}와의 연락이 갑자기 잦아지거나, 만나자는 말이 가벼워지는 때가 바로 그 공기예요. 예쁘게 보이는 신호에 취해 확인을 미루면, 좋은 달이 지나간 뒤에야 아쉬움이 남아요.`
        : `상대가 없어도 걱정 마세요. 평소 안 가던 모임, 미뤄 둔 소개, 갑자기 길어지는 대화창. 그런 입구가 ${f.dohwaMonth} 전후에 더 자주 열리네.`,
      actions: [
    ...
    'love-spouse-star': {
      ...
      scene: f.hasPartner
        ...
        : `새 인연 앞에서도 같아요. 대화가 화려해도 일정이 비고, 말은 담백해도 약속이 쌓이는 사람. 후자가 ${f.name} 사주에서 더 깊게 남는 결이에요.`,
      actions: [
        '상대를 고를 때 "설렘"과 "안심"을 나란히 적어 보세요',
    ...
    'love-monthly-flow': {
      ...
      scene: `${f.relation}이라면 같은 달에도 봐야 할 장면이 다르네. 솔로는 입구, 썸은 약속, 연애 중은 리듬, 이별 뒤에는 재회보다 반복되는 미련. ${f.dohwaMonth}가 겹치면 그 장면이 더 또렷해질 거예요.`,
      actions: [
        '이번 달의 연애 목표를 "만나기 / 확인하기 / 정리하기" 중 하나로만 정하세요',
    ...
    'love-progress-timing': {
      ...
      scene: f.hasPartner
        ? `${f.partnerName}에게 "우리 뭐야"보다 "다음엔 뭐 할까"가 더 잘 닿을 수 있어요. 감정을 증명하라 몰아붙이면 온도가 떨어지고, 현실을 하나 정해 주면 마음이 따라오는 타입이 있거든.`
        : `상대가 없다면, 소개받은 자리에서 호감을 숨기지 마세요. 다만 전부를 쏟지 말고, 두 번째 만남을 만드는 데 힘을 쓰게.`,
      actions: [
    ...
    'love-missed-signals': {
      ...
      softBridge: `마음이 급할수록, 작은 침묵을 더 크게 읽게 되는 법이에요. 위험은 큰 사건보다 반복된 실수에 있어요.`,
      tableMd: [
        '| 위험 신호 | 그때의 마음 |',
        '| --- | --- |',
      ...
      scene: `도화가 강한 달일수록 이 실수는 더 예뻐 보이네. 불이 붙으니 확인이 사소해 보이고, 놓치면 안 될 것 같아 서두르게 되지.  그때일수록 한 호흡만 늦추게.`,
      actions: [
    ...
    'love-partner-compatibility': {
      ...
      tableCaption: '궁합은 표로 재는 게 아니라, 장면으로 확인하는 거예요.',
      scene: f.hasPartner
        ? `${f.partnerName}(${f.partnerRelation})와의 온도 차는 답장 횟수보다, 확인했을 때 피하지 않는가에서 드러나요. 피하면 마음이 없는 게 아니라, 속도가 다른 것일 수도 있어요. 그 차이를 존중하되, ${f.name}이 사라지지는 마세요.`
        : `새 사람을 볼 때는 첫 설렘의 크기보다 세 번째 만남 이후의 편안함을 믿게. 첫인상은 도화가 만들고, 오래감은 일상의 결이 만드네.`,
      actions: [
        f.hasPartner ? `${f.partnerName}와 최근 약속을 한 줄로 정리해 보세요` : '새 인연에게 세 번째 만남까지 기준을 미뤄 두게',
        '속도 차이가 느껴지면 비난 대신 리듬을 맞춰 보세요',
    ...
    'love-emotion-temperature': {
      hook,
      feel: `사랑의 크기가 같아도 감정 온도는 다를 수 있어요. ${f.name}은 마음이 정해지면 답을 보고 싶어하고, 상대는 같은 마음이라도 표현 속도를 늦추며 거리감을 고를 수 있지.`,
      ...
      tableCaption: '온도 차이에서 생기는 착각만 먼저 걷어 볼게요.',
      scene: f.hasPartner
        ? `${f.partnerName}와의 사이에서는 연락 빈도보다 반복성을 보세요. 서운할 때마다 사라지고 돌아오는 패턴인지, 천천히라도 쌓이는 패턴인지. 후자라면 온도가 달라도 관계는 자랄 수 있어요.`
        : `특정 상대가 없을 때는 처음부터 많은 확신을 요구하지 마세요. 확신을 너무 일찍 구하면, 상대의 온도가 미처 올라오기도 전에 문이 닫히네.`,
      actions: [
        '불안할 때 연락 횟수 대신 "반복되는 배려"를 세어 보세요',
    ...
    'love-action-strategy': {
      hook,
      feel: `마지막으로 연애 성사 전략이네. 운을 기다리는 해로만 쓰지 마세요. 소개를 열어 두고, 약속을 만들고, 마음이 흔들릴 때 행동의 속도를 고르는 해로 쓰게.`,
      softBridge: softUseful,
      tableMd: [
        '| 시기 | 할 일 |',
        '| --- | --- |',
      ...
      tableCaption: '성사 전략은 거창한 작전보다, 달에 맞는 작은 용기입니다.',
      chartPoints: chart,
      chartCaption: '행동 타이밍을 겹쳐 보면, 기회가 보이는 달이 드러나요.',
      scene: f.hasPartner
        ? `${f.partnerName}가 있다면 감정을 증명하라 몰아붙이기보다, 같이 할 일 하나를 정하세요. 그 약속이 두 번 반복되면, 말은 따라오게 되어 있어요.`
      ...
      actions: [
        ...
        '매주 일요일, 이번 주 놓친 신호를 한 줄로만 돌아보세요',
      ],
      imagePrompt: {
  ...
  const selected = beats[sectionId] ?? {
    hook,
    feel: `${f.name}님의 올해 연애 이야기부터 천천히 펼쳐 볼게요. 마음이 먼저 묻는 장면이 곧 입구예요.`,
    softBridge: softYear,
    scene: `일상의 작은 반복 속에 인연의 신호가 숨어 있어요. 서두르지 말고, 놓치지만 마세요.`,
