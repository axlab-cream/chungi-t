...
/**
 * `reviewPaidSectionDensity` 의 다음 판단 기준 게이트는 시간 표지와 확인 대상과 행동
 * 서술어가 **한 문장 안에** 함께 있을 때만 통과한다. 재작성 지시문만 이 문장 형태를
 * 요구하고 1차 지시문은 요구하지 않아서, 모든 서비스가 1차에서 같은 항목으로 떨어지고
 * 재작성 한 번으로도 회복하지 못했다(2026-09-17). 두 지시문이 이 상수를 공유해야
 * 1차부터 같은 문장을 쓰고 둘이 갈라지지 않는다.
 */
const SECTION_CLOSING_RULES = [
  '마지막 의미 단락은 2~4개의 완성 문장으로 쓰고, 그중 한 문장에 시간 표지와 확인 대상과 행동 서술어를 함께 담으세요. “다음에는 카드 명세서에서 구독료 항목을 비교해요.” 또는 격식체라면 “다음에는 업무 요청별 책임 범위를 기록하십시오.”처럼 다음에는·앞으로·이후·먼저·오늘 가운데 하나와, 무엇을 볼지 가리키는 목적어와, 기록·비교·확인·유지 가운데 하나의 서술어가 한 문장 안에 모두 있어야 합니다.',
...
function sectionRepairInstruction(issues: string[], mode: 'rewrite' | 'edit' = 'rewrite'): string {
  ...
  const guidance = [
    ...
    // 지적받은 요소를 고치면서 이미 통과했던 요소를 떨어뜨리는 왕복이 실제로 반복됐다.
    // 네 요소를 매번 함께 확인하도록 묶어 둔다.
    '아래 네 가지는 한 응답 안에 모두 있어야 합니다. 하나를 고치면서 나머지를 빼지 마세요. (1) hook에 질문의 직접 답, (2) 그 답을 만든 개인 근거 또는 판단 조건, (3) 독자가 자기 일상에서 알아볼 수 있는 생활 장면, (4) 마지막 단락의 다음 판단 기준.',
    '첫 문장부터 질문에 직접 답하고, 사용자 사실과 검증된 계산은 근거로만 쓰며 전통적 상징은 해석 후보로 구분하세요.',
    '실제 경험이 아니면 “예를 들어”라고 밝히고, 장소 또는 도구와 관찰 행동이 함께 있는 알아볼 수 있는 장면을 쓰세요. 출근·퇴근·회의·답장·연락·약속·대화·책상·식탁·냉장고·결제·지출·주말처럼 구체적인 자리나 물건을 “~할 때”, “~하면”, “~인 경우”와 함께 쓰세요.',
    ...SECTION_CLOSING_RULES,
...
export async function buildOpenAiSajuReportSection(
  ...
  const raw = await chatWithOpenAI(messages, {
    model: REPORT_MODEL,
    /**
     * gpt-5 계열은 추론 토큰을 이 예산에서 함께 깎는다. 통과한 항목의 본문이 952~1585 토큰인데
