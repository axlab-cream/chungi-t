...
export function reviewInterpretation(text: string, context: SajuReportContext, siblings: SajuReportSection[] = []): InterpretationReview {
  ...
  /*
   * 운영 문구 노출. `결제 전`·`진행 중` 은 2026-09-18 까지 단독으로 걸었는데, 저축운 본문의
   * "다음 결제 전에 남은 한도를 비교해요", "적금 진행 중이라면" 같은 고객 문장이 오탐으로
   * 떨어져 1차 실패의 4할을 만들었다. 운영 맥락(결제 상태·열람 권한·생성 상태)과 붙어 있을 때만 건다.
   */
  const opsPaymentGate = /결제\s*전(?:에는|엔|까지는|이라)?\s*(?:볼|열|확인할|읽을|공개|잠겨|이용|미리보기|상세)|(?:결제|주문|승인)\s*(?:상태|확인)\s*(?:진행|대기)\s*중/
  const opsGenerationState = /(?:해석|풀이|리포트|생성|준비|분석)\s*(?:이|가)?\s*진행\s*중|진행\s*중(?:입니다|이에요|이예요)?\s*[.…]?\s*(?:잠시|곧|완료되면|기다려)/
  if (/풀이\s*\d+|에서 보는 핵심|확인한 기준|이 풀이에 반영한 정보|상세는 서버 권한|서버 권한 확인|해석을 준비|로그인과 결제 상태|취소 또는 실패/.test(text) || opsPaymentGate.test(text) || opsGenerationState.test(text)) issues.push('제작용 제목·운영 상태·권한/결제 안내를 고객 해석 본문에 노출하지 마세요.')
