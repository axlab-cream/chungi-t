...
interface DirectorySeed {
  ...
  image: string
  /**
   * The service's own 06-1 detail page - where a saved reading reopens so it keeps the
   * design it was sold in. Absent means the service has no such page yet and the reading
   * falls back to the generic reader at /r/:id.
   */
  reportPath?: string
...
/**
 * Readings saved before a service key was renamed still carry the old key, so the lookup
 * goes through one table both callers share - a second copy is how the two drift apart.
 */
