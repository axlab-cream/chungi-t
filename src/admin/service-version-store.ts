...
function stableDraftJson(value: unknown): string {
  if (Array.isArray(value)) return `[${value.map(stableDraftJson).join(',')}]`
  if (value && typeof value === 'object') return `{${Object.entries(value as Record<string, unknown>).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => `${JSON.stringify(key)}:${stableDraftJson(item)}`).join(',')}}`
  return JSON.stringify(value)
}

function checksum(fields: ServiceDraftFields): string {
  return createHash('sha256').update(stableDraftJson(fields)).digest('hex')
}
