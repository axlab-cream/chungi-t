...
test('택일 목차는 6 대분류 12 중분류를 유지한다', () => {
  // 2026-09-17 목차 축소: 20 → 12 (대분류당 2개). 05/06 은 API 응답을 그대로 그리므로
  // (하드코딩된 id 목록 없음) 여기 개수만 맞으면 화면도 그대로 따라온다.
  assert.equal(WEDDING_TOC.length, 6)
  assert.equal(allItems.length, 12)
  // 05 목차와 06 상세가 이 id 로 라우팅하므로 중복이 있으면 상세가 엉뚱한 항목을 연다.
  assert.equal(new Set(allItems.map((item) => item.id)).size, 12)
...
test('무료 티저는 후보일 판정을 앞세우고 유료 범위를 숨기지 않는다', () => {
  ...
  assert.equal(teaser.scope.reduce((n, group) => n + group.items.length, 0), 12)
})
