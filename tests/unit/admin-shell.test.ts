...
describe('관리자 셸 (T07)', { concurrency: false }, () => {
  ...
  describe('정상 동작', () => {
    ...
    it('관리자 메뉴는 좌측 LNB와 모든 운영 화면 경로를 제공한다', async () => {
      ...
      assert.match(text, /data-notice-diff/, '발행본 대비 diff가 없다')
      assert.match(text, /data-notice-preview/, '고객 노출 미리보기가 없다')
      assert.match(text, /data-notice-request-approval/, '승인 요청 CTA가 없다')
      assert.match(text, /data-notice-approve/, '승인 CTA가 없다')
      assert.match(text, /data-notice-schedule/, '예약 CTA가 없다')
      assert.match(text, /data-notice-cancel-schedule/, '예약 취소 CTA가 없다')
