...
describe('관리자 셸 (T07)', { concurrency: false }, () => {
  ...
  describe('정상 동작', () => {
    ...
    it('셸이 직원 로그인 폼을 갖고 있다', async () => {
      ...
      assert.match(text, /signInWithPassword/, '자체 비밀번호 실패 뒤 Supabase 비밀번호 인증 폴백이 없다')
      assert.match(text, /\/api\/admin\/v1\/session\/supabase/, 'Supabase 인증을 관리자 HttpOnly 세션으로 교환하지 않는다')
      const passwordClientSource = text.slice(text.indexOf('function adminPasswordClient()'), text.indexOf('async function currentSession()'))
      assert.match(passwordClientSource, /persistSession:\s*false/, '관리자 비밀번호 인증 세션을 브라우저에 저장한다')
      assert.match(passwordClientSource, /autoRefreshToken:\s*false/, '일회성 관리자 인증 토큰을 자동 갱신한다')
      assert.match(text, /fallbackClient\.auth\.signOut\(\{ scope: 'local' \}\)/, '로그인 교환 뒤 일회성 Supabase 세션을 지우지 않는다')
      ...
      const startSource = text.slice(text.indexOf('async function start()'), text.indexOf('\n      start();'))
      assert.ok(startSource.indexOf('if (isRecoveryLink())') < startSource.indexOf('if (config.adminLocalAuth)'), '로컬 관리자 모드가 복구 링크 처리를 가로막는다')
      assert.ok(!text.includes('rememberDeviceAuthSession(session)'), '복구용 Supabase 세션을 브라우저에 기억한다')
      assert.match(text, /recoveryClient\.auth\.signOut\(\{ scope: 'local' \}\)/, '복구 완료 뒤 Supabase 세션을 지우지 않는다')
      const signOutSource = text.slice(text.indexOf('function bindSignOut()'), text.indexOf('async function openPublicOrders()'))
      assert.match(signOutSource, /if \(config && !config\.adminLocalAuth\)/, '로컬 관리자 로그아웃이 무관한 고객 Supabase 세션까지 지울 수 있다')
