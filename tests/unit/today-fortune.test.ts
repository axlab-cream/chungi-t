...
describe('[TASK] 오늘운 테스트 하네스', () => {
  it('저장된 사용자 사주 프로필로 KST 기준 오늘운을 만든다', () => {
    ...
    assert.equal(fortune.reading.details.caution.text, fortune.reading.caution)
    assert.match(fortune.reading.summary, /상징|실제 사건/)
    assert.doesNotMatch(fortune.reading.relationship, /반응이 부드럽습니다|먼저 열면/)
  })
})
