...
test('lucky color visual QA source is one immutable complete 24-section isolated record', () => {
  const evidence = JSON.parse(read('tone-v2/evaluations/P04-lucky-color-full-outline-generation-20260913.json'))
  assert.equal(evidence.status, 'pass')
  assert.equal(evidence.serviceKey, 'lucky_color')
  assert.deepEqual(evidence.completion, {
    completedSections: 24, expectedSections: 24, recordStatus: 'complete', firstFailure: null,
    attemptedAfterFailure: 0, attemptedOutsideLimit: 0,
  })
...
test('lucky color actual-record QA server stays local, read-only and fail-closed', () => {
  const path = 'scripts/qa-lucky-color-live-reader.ts'
  assert.equal(existsSync(join(root, path)), true)
  const source = read(path)
  assert.match(source, /P04-lucky-color-full-outline-generation-20260913\.json/)
  assert.match(source, /reading-live-20260907\/records/)
  assert.match(source, /recordPath\.startsWith/)
  assert.match(source, /serviceKey !== 'lucky_color'/)
  assert.match(source, /sections\.length !== 24/)
  assert.match(source, /developmentReportAccess: true/)
  assert.match(source, /127\.0\.0\.1/)
  assert.match(source, /qa\/lucky-color-mobile-frame/)
