...
describe('결제 자격 판정 — 미결속 주문 claim', { concurrency: false }, () => {
  it('미결속 주문은 처음 여는 풀이에 결속되고, 두 번째 풀이는 열지 못한다', async () => {
    ...
    const first = await settleOrderAccess(seeded, OWNER, PRODUCT, { reportId: 'report-a' })
    ...
    assert.equal(await settleOrderAccess(stored!, OWNER, PRODUCT, { reportId: 'report-b' }), null, '같은 주문으로 다른 풀이를 열 수 없다')
  })

  it('주문 ID 를 직접 대도 다른 풀이로 재사용되지 않는다', async () => {
    const seeded = await seedUnbound()
    assert.ok(await settleOrderAccess(seeded, OWNER, PRODUCT, { reportId: 'report-a' }))
    const replayed = await getPaymentOrder(seeded.orderId)
    assert.equal(await settleOrderAccess(replayed!, OWNER, PRODUCT, { reportId: 'report-c' }), null)
  })

  it('서로 다른 풀이를 동시에 열면 한쪽만 통과한다', async () => {
    const seeded = await seedUnbound()
    const settled = await Promise.all([
      settleOrderAccess(seeded, OWNER, PRODUCT, { reportId: 'race-a' }),
      settleOrderAccess(seeded, OWNER, PRODUCT, { reportId: 'race-b' }),
    ])
  ...
  it('컷오프 이전 레거시 주문은 claim 없이 통과하고 결속되지 않는다', async () => {
    ...
    assert.ok(await settleOrderAccess(seeded, OWNER, PRODUCT, { reportId: 'report-old', createdAt: BEFORE_CUTOFF }), '레거시 구매자는 그대로 열려야 한다')
    const stored = await getPaymentOrder(seeded.orderId)
  ...
  it('다른 계정은 남의 미결속 주문으로 열 수 없다', async () => {
    const seeded = await seedUnbound()
    assert.equal(await settleOrderAccess(seeded, { id: 'owner-2' }, PRODUCT, { reportId: 'report-a' }), null)
    const stored = await getPaymentOrder(seeded.orderId)
