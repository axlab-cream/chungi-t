/** Isolated, paid synthetic saju_master full-outline provider check. */
...
process.env.REPORT_STORAGE_DIR = resolve('.cache/reading-live-20260914-saju-master')
...
const maxSections = rawLimit === undefined ? 37 : Number(rawLimit)
if (!Number.isInteger(maxSections) || maxSections < 1 || maxSections > 37) throw new Error('--limit must be an integer from 1 through 37')

const birth = { year: 1991, month: 4, day: 18, hour: 11, gender: 'female' as const, calendar: 'solar' as const }
const context = {
  serviceKey: 'saju_master',
  name: '합성 천명사주 전체 목차 점검',
  target: 'self',
  relationship: 'single',
  work: 'employed',
  concern: '현재 일과 관계에 큰 문제는 없습니다. 잘 유지되는 조건과 선택 기준을 확인하고 싶습니다.',
  birthTimeKnown: true,
}
const analysis = analyzeSaju(birth)
const reportId = createReportId(birth, context, `live-qa-${version}`)
let record = await getReportRecord(reportId)
if (requireFresh && record) throw new Error(`Fresh saju_master outline version already exists: ${version}`)

if (shouldGenerate) {
  const templateReport = buildTemplateSajuReport(analysis, birth, context)
  assert.equal(templateReport.sections.length, 37)
  const created = await createOrGetReportRecord({ reportId, birth, context, analysis, templateReport })
  if (requireFresh && !created.created) throw new Error(`Fresh saju_master outline did not create a new record: ${version}`)
  console.log(JSON.stringify({ phase: 'saved-before-model', version, reportId, resultId: created.record.resultId, total: 37, created: created.created }))
  for (const section of created.record.report.sections.slice(0, maxSections)) {
    ...
    console.log(JSON.stringify({ phase: 'section-finished', order: section.order, id: section.id, status: generated.status, complete, total: 37, attempts: generated.attempts?.length ?? 0 }))
...
if (recoverSectionId) {
  if (!record) throw new Error(`Cannot recover a missing saju_master outline version: ${version}`)
...
if (retrySectionId) {
  if (!record) throw new Error(`Cannot retry a missing saju_master outline version: ${version}`)
...
if (!record) {
  console.log(JSON.stringify({ version, service: 'saju_master', status: 'not-generated' }))
  process.exit(0)
}
...
if (failedIndex < 0 && maxSections === 37 && (shouldGenerate || (!retrySectionId && !recoverSectionId))) assert.equal(client.progress.complete, 37)
...
const sections = record.report.sections.map((section, index) => {
  ...
  const replay = section.status === 'complete' ? reviewGeneratedSajuReportSection({
    analysis: record!.analysis ?? analysis, birth: record!.birth, context: record!.context, section,
    hook: section.hook, interpretation: section.interpretation, siblings: previous, corpusSnapshot: record!.corpus,
  }) : undefined
  return {
...
const replayFailures = sections.filter((section) => section.status === 'complete' && section.replay && !section.replay.passed)
assert.equal(replayFailures.length, 0, 'Every completed section must pass production-equivalent replay')
console.log(JSON.stringify({
  version, service: 'saju_master', requestedLimit: maxSections, syntheticOnly: true,
  isolatedStorage: process.env.REPORT_STORAGE_DIR, reportId, resultId: client.resultId,
  recordStatus: record.status, progress: client.progress,
  firstFailure: failedIndex < 0 ? null : { order: record.report.sections[failedIndex].order, id: record.report.sections[failedIndex].id },
  attemptedAfterFailure: attemptedAfterFailure.length, attemptedOutsideLimit: attemptedOutsideLimit.length,
  recordSha256: createHash('sha256').update(JSON.stringify(record)).digest('hex'), sections,
}, null, 2))
