// Asserts every integration point of the 나 지금 그만둬도 될까? (couple_signal) service is wired.
//
// Same guard as check-marry.mjs and check-save.mjs. The 목차 is the fragile part here too:
// 05 목차의 열 줄과 06 상세의 카드가 모두 LOVE_SIGNAL_TOC의 대분류 수와 section id에 묶여 있어서,
...
const SERVICE_DIR = '사주/love/signal'
const ASSET_DIR = `${SERVICE_DIR}/assets/signal`
...
const CONTRACTS = [
  ['src/server/app.ts', "app.get(['/love/signal',", '진입 라우트'],
  ['src/server/app.ts', "'/love/signal/input'", '02 입력 라우트'],
  ['src/server/app.ts', "'/love/signal/report'", '04 티저 라우트'],
  ['src/server/app.ts', "'/love/signal/chat'", '05 목차 라우트'],
  ['src/server/app.ts', "'/love/signal/detail'", '06 상세 라우트'],
  ['src/server/app.ts', "app.post('/api/love/signal/analyze'", '분석 API'],
  ['src/server/app.ts', "couple_signal: 'couple_signal'", '결제 상품 매핑'],
  ['src/payment/catalog.ts', 'couple_signal: {', '결제 카탈로그 등록'],
  ['src/payment/catalog.ts', "returnPath: '/love/signal/04-step-4-report/index.html'", '결제 복귀 경로'],
  ['src/love/signal-service.ts', "export const LOVE_SIGNAL_SERVICE_KEY = 'couple_signal'", '서비스 키 상수'],
  ['src/love/signal-service.ts', 'export const SIGNAL_ASSET_BASE', '아트워크 경로 상수'],
  ['src/love/signal-service.ts', 'export const LOVE_SIGNAL_TOC', '목차 export'],
  ['src/love/signal-service.ts', 'LOVE_SIGNAL_TOC', '대분류 목차'],
  ['src/love/signal-service.ts', './reading-content.js', '대분류별 해석 문장 테이블'],
  ['src/love/signal-service.ts', "const OWN_CORPUS_DOMAIN = 'couple_signal_service'", '코퍼스 도메인 게이트'],
  ['src/love/signal-service.ts', 'retrieveCategoryOwnChunks', 'RAG 근거 추출'],
  ['사주/js/signal-service.js', "'/api/love/signal/analyze'", '프론트 리포트 호출'],
  ['사주/js/signal-service.js', 'function enhanceSajuInput', '저장된 사주 재사용'],
  ['사주/js/signal-service.js', 'function mountChrome', '공통 GNB 마운트'],
  ['사주/portal.html', 'href="/love/signal"', '포털 카드 연결'],
]
...
const serviceBody = read('src/love/signal-service.ts')
if (serviceBody !== null) {
  const start = serviceBody.indexOf('export const LOVE_SIGNAL_TOC')
...
for (const [page, needsChrome] of PAGES) {
  ...
  if (needsChrome) {
    if (!body.includes('/js/signal-service.js')) failures.push(`${rel} :: 서비스 스크립트 미연결`)
...
if (failures.length === 0) {
  console.log('couple_signal 연동 지점 전부 정상 (%d개 계약, %d개 페이지, %d개 대분류)', CONTRACTS.length, PAGES.length, EXPECTED_GROUPS)
  process.exit(0)
}

console.error(`couple_signal 연동 손상 ${failures.length}건:`)
