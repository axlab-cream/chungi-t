/**
 * Paid, opt-in synthetic lucky_color full-outline check.
 ...
 *
 * npx tsx scripts/check-lucky-color-outline-live.ts --version=<unique> --generate --fresh [--limit=1]
 * npx tsx scripts/check-lucky-color-outline-live.ts --version=<unique> --retry-section=<section-id>
 * npx tsx scripts/check-lucky-color-outline-live.ts --version=<unique> --recover-section=<section-id>
 * npx tsx scripts/check-lucky-color-outline-live.ts --version=<unique>
 */
...
} = await import('../src/body/lucky-service.js')
...
const context = {
  ...buildLuckyColorContext('합성 색과 물건 전체 목차 점검', input),
...
if (requireFresh && record) throw new Error(`Fresh lucky_color outline version already exists: ${version}`)
...
if (recoverSectionId) {
  if (!record) throw new Error(`Cannot recover a missing lucky_color outline version: ${version}`)
...
if (retrySectionId) {
  if (!record) throw new Error(`Cannot retry a missing lucky_color outline version: ${version}`)
...
if (!record) {
  console.log(JSON.stringify({ version, service: LUCKY_COLOR_SERVICE_KEY, status: 'not-generated' }))
  process.exit(0)
}
...
console.log(JSON.stringify({
  version,
  service: LUCKY_COLOR_SERVICE_KEY,
