...
const CONTRACTS = [
  ...
  // 궁별 문장이 practical-readings.ts 로, 상징 산출이 workSymbol 로 옮겨졌다.
  ['src/work/jobchoice-service.ts', 'JOB_CHOICE_TOC', '대분류 목차'],
  ['src/work/jobchoice-service.ts', './practical-readings.js', '대분류별 해석 문장 테이블'],
  ['src/work/jobchoice-service.ts', 'workSymbol', '궁 상징 산출'],
  ['src/report/practical-service-copy.ts', 'export function workSymbol', '궁 상징 계산'],
  ['src/work/jobchoice-service.ts', "const OWN_CORPUS_DOMAIN = 'job_choice_service'", '코퍼스 도메인 게이트'],
  ['src/work/jobchoice-service.ts', 'retrieveCategoryOwnChunks', 'RAG 근거 추출'],
