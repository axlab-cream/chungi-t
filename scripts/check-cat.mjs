...
const CONTRACTS = [
  ...
  // fdc80f2 로 대분류별 문장이 cat-practical-readings.ts 로 옮겨졌다.
  ['src/pet/cat-service.ts', 'CAT_COMPAT_TOC', '대분류 목차'],
  ['src/pet/cat-service.ts', './cat-practical-readings.js', '대분류별 해석 문장 테이블'],
  ['src/pet/cat-service.ts', "const OWN_CORPUS_DOMAIN = 'cat_compatibility_service'", '코퍼스 도메인 게이트'],
  ['src/pet/cat-service.ts', 'retrieveCategoryOwnChunks', '자기 코퍼스 우선 검색'],
  ['src/pet/cat-service.ts', 'retrieveCategoryRagChunks', 'RAG 근거 추출'],
  ['src/report/practical-service-copy.ts', 'export function practicalReading', '근거 기반 문장 생성'],
