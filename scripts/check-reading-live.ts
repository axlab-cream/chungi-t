 ...
 * npx tsx scripts/check-reading-live.ts --generate [--serve]
...
// Keep only the approved provider credential; isolate customer storage/auth/payment entirely.
for (const name of Object.keys(process.env)) if (/DATABASE|SUPABASE|INICIS|PAYMENT|VERCEL/.test(name)) delete process.env[name]
...
const { reviewInterpretation } = await import('../src/report/interpretation-validation.js')
...
for (const [index, context] of contexts.entries()) {
  ...
  if (process.argv.includes('--generate')) {
    ...
    console.log(JSON.stringify({ phase: 'saved-before-model', service: context.serviceKey, resultId: created.record.resultId }))
  ...
  summary.push({ service: context.serviceKey, reportId, resultId: client.resultId, url: `http://127.0.0.1:8793${client.publicUrl}`, status: record.status, model: section.model, review: reviewInterpretation(section.interpretation, context), attempts: section.attempts?.map((item) => ({ id: item.id, status: item.status, finishReason: item.finishReason, usage: item.tokenUsage, error: item.error })) })
}
