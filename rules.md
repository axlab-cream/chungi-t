...
| 코드 | 조건 |
|------|------|
| H1 | force push, 브랜치 삭제, PR 머지, 또는 사용자가 명시적으로 요청하지 않은 범위의 원격 git push |
| H2 | DNS·TLS·nginx·systemd 변경, DB 마이그레이션, 또는 사용자가 명시적으로 요청하지 않은 범위의 배포 실행 |
| H3 | 시크릿(.env, 키, 토큰) 외부 전송 |
| H4 | 세션 이전 파일의 비가역적 삭제 |
| H5 | 사용자가 명시적으로 요청하지 않은 본 §6 자체의 변경 |

해당 작업은 수행하지 않고 status.md에 [GATE]로 기록한 뒤
다음 이슈로 진행한다. 전체 실행을 중단하지 않는다.

사용자가 특정 서비스의 UI·UX 변경을 커밋·배포하도록 명시적으로 요청하면,
검증된 해당 변경만 일반 원격 push와 기존 배포 경로로 반영할 수 있다.
실행 전 대상 브랜치·변경 파일·검증 결과·복구 경로를 확인하고,
실행 후 배포 버전과 실제 운영 화면을 확인한다.
관련 없는 사용자 변경, 팝업 및 팝업 관리자 등 요청 범위 밖 기능은 보존한다.
이 예외는 force push, 브랜치 삭제, PR 머지, 인프라 변경, DB 마이그레이션,
시크릿 외부 전송 또는 비가역적 삭제를 허용하지 않는다.
