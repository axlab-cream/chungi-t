...
- 2페이지: `남부대공 · 사주` 사주 등록 단계. 1페이지 CTA 이후 `#target` 상태로 `누구의 사주를 볼까요?` 화면부터 보여야 한다.
...
- 샘플의 `범산 도령이다` 문구는 프로젝트 페르소나에 맞춰 `남부대공이다`로 치환했다.
...
- 사용자 샘플 브랜드명 `범산 도령`과 현재 서비스명 `남부대공`은 다르므로, 텍스트 1:1이 아니라 페르소나명만 서비스명으로 치환했다.
