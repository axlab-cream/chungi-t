...
>
> **[2026-09-10 16:25 후속]** U14가 정정되었으나 **이 항목은 여전히 미확인이다.**
> Git-triggered Vercel 빌드가 이 로컬 명령을 자동 실행한다는 증거는 없다.
> 그리고 과거 배포가 이 검사를 실행했는지도 명령 기록·CI 기록이 없어 알 수 없다.
> 확정된 것은 이 검사의 성격뿐이다 — 배포 차단 장치가 아니라 **사람이 push 전에 돌리는
> 게이트**이며, 스스로 "does not intercept other deploys"라고 밝힌다. (TASK-018 §0·§8)
